...
def read_str(data, offset, length):
    ...
    if offset + length > data_length:
        raise InvalidOffsetError(
            f"Offset 0x{offset:X}, length 0x{length:X} is past the end of the data (length 0x{data_length:X})."
        )
    data.seek(offset)
...
def read_str_until_null_character(data, offset):
    ...
    if offset > data_length:
        raise InvalidOffsetError(
            f"Offset 0x{offset:X} is past the end of the data (length 0x{data_length:X})."
        )
...
def write_str(data, offset, new_string, max_length):
    ...
    if str_len >= max_length:
        raise Exception(
            f'String "{new_string}" is too long (max length including null byte: 0x{max_length:X})'
        )
...
def write_magic_str(data, offset, new_string, max_length):
    ...
    if str_len > max_length:
        raise Exception(
            f"String {new_string} is too long (max length 0x{max_length:X})"
        )
