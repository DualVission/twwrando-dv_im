...
class WWRandomizerWindow(QMainWindow):
    ...
    def __init__(self, cmd_line_args=OrderedDict()):
        ...
        self.ui.randomize_all_custom_colors_together.clicked.connect(
            self.randomize_all_custom_colors_together
        )
        self.ui.randomize_all_custom_colors_separately.clicked.connect(
            self.randomize_all_custom_colors_separately
        )
        self.ui.custom_color_preset.currentIndexChanged.connect(
            self.color_preset_changed
        )
        ...
        self.ui.sword_mode.highlighted.connect(
            self.update_sword_mode_highlighted_description
        )
    ...
    def remove_from_starting_gear(self):
        ...
        self.ui.randomized_gear.model().sourceModel().sort(
            0, key=lambda x: SORT_KEY.index(x)
        )
        self.update_settings()
    ...
    def randomize(self):
        ...
        if not os.path.isfile(clean_iso_path):
            QMessageBox.warning(
                self,
                "Clean ISO path not specified",
                "Must specify path to your clean Wind Waker ISO (USA).",
            )
            return
        ...
        if self.bulk_test:
            ...
            for i in range(100):
                ...
                try:
                    ...
                    while True:
                        next_option_description, options_finished = next(
    ...
    def randomization_failed(self, error_message):
        ...
        print(error_message)
        QMessageBox.critical(self, "Randomization Failed", error_message)

    def show_update_check_results(self, new_version):
        if not new_version:
            self.ui.update_checker_label.setText(
                "No new updates to the randomizer are available."
            )
            ...
            new_text += (
                ' <a href="%s">Click here</a> to go to the download page.'
                % LATEST_RELEASE_DOWNLOAD_PAGE_URL
            )
    ...
    def load_settings(self):
        ...
        if "custom_colors" in self.settings:
            ...
            for (
                color_name,
                default_color,
            ) in self.get_default_custom_colors_for_current_model().items():
                if color_name in custom_colors_from_settings:
                    self.custom_colors[color_name] = custom_colors_from_settings[
                        color_name
                    ]
                else:
            ...
            for color_name in self.get_default_custom_colors_for_current_model():
                ...
                self.set_color(
                    option_name, color, update_preview=False, save_color_as_custom=False
                )

        if "custom_color_preset" in self.settings:
            self.set_option_value(
                "custom_color_preset", self.settings["custom_color_preset"]
            )
            self.reload_colors()
    ...
    def update_total_progress_locations(self):
        ...
        for option_name in OPTIONS:
            options[option_name] = self.get_option_value(option_name)
        num_progress_locations = Logic.get_num_progression_locations_static(
            self.cached_item_locations, options
        )
        if self.get_option_value("race_mode") == "Race":
            try:
                num_remove = (
                    84 - int(self.get_option_value("num_dungeon_race_mode")) * 14
                )
            except:
                num_remove = 14 * 5
        ...
        text = (
            "Where Should Progress Items Appear? (Selected: %d Possible Progression Locations)"
    ...
    def permalink_modified(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Invalid permalink", "The permalink you pasted is invalid."
            )
    ...
    def encode_permalink(self):
        ...
        for option_name in OPTIONS:
            ...
            if option_name == "randomize_enemy_palettes" and not self.get_option_value(
                "randomize_enemies"
            ):
    ...
    def decode_permalink(self, base64_encoded_permalink):
        ...
        prev_randomize_enemy_palettes_value = self.get_option_value(
        ...
        for option_name in OPTIONS:
            ...
            if isinstance(widget, QAbstractButton):
                ...
                for i in range(len(STNDRD_ITEMS)):
                    ...
                    if starting == 1:
                        self.ui.randomized_gear.selectionModel().select(
                            self.randomized_gear_model.index(i),
        ...
        if not self.get_option_value("randomize_enemies"):
            ...
            self.set_option_value(
                "randomize_enemy_palettes", prev_randomize_enemy_palettes_value
            )

        self.update_settings()

    def browse_for_clean_iso(self):
        if self.settings["clean_iso_path"] and os.path.isfile(
        ...
        clean_iso_path, selected_filter = QFileDialog.getOpenFileName(
            self,
            "Select clean Wind Waker ISO",
            default_dir,
            "GC ISO Files (*.iso *.gcm)",
        )
    ...
    def browse_for_output_folder(self):
        if self.settings["output_folder"] and os.path.isdir(
    ...
    def set_option_value(self, option_name, new_value):
        ...
        if isinstance(widget, QCheckBox) or isinstance(widget, QRadioButton):
            ...
            if new_value < widget.minimum() or new_value > widget.maximum():
                ...
                new_value = self.default_settings[
                    option_name
                ]  # reset to default in case 0 is not default or in normal range
    ...
    def update_color_presets_list(self, reload_colors=True):
        ...
        for preset_name in presets:
            if preset_name in ["Default", "Custom"]:
                QMessageBox.warning(
                    self,
                    "Invalid color preset name",
                    'The selected player model has a preset named "%s", which is a reserved name. This preset will be ignored.'
                    % preset_name,
                )
                continue
    ...
    def reload_custom_model(self, update_preview=True):
        ...
        if "error_message" in metadata:
            error_message = (
                "Syntax error when trying to read metadata.txt for custom model: %s\n\n%s"
                % (custom_model_name, metadata["error_message"])
            )
            print(error_message)
            QMessageBox.critical(self, "Failed to load model metadata", error_message)
        ...
        if custom_model_name == "Link":
            ...
            comment_lines.append(
                "Selects from model list and uses default color scheme."
            )
        ...
        if custom_model_name != "Link":
            ...
            if (
                os.path.isfile(jaiinit_aaf_path)
                or os.path.isfile(voice_aw_path)
                or custom_model_name in ["Random", "Random (exclude Link)"]
            ):
                self.ui.disable_custom_player_voice.show()
            else:
            ...
            if (os.path.isfile(ship_arc_path)) or (
                custom_model_name in ["Random", "Random (exclude Link)"]
            ):
    ...
    def reset_color_selectors_to_model_default_colors(self):
        ...
        for custom_color_name, default_color in default_colors.items():
            if (
                custom_color_name in self.custom_colors
                and self.custom_colors[custom_color_name] != default_color
            ):
    ...
    def ensure_valid_combination_of_options(self):
        ...
        if self.get_option_value("race_mode") != "Default":
            try:
                num_possible_rewards = 8 - int(
                    self.get_option_value("num_starting_triforce_shards")
                )
            except:
        ...
        all_gear = self.get_option_value("starting_gear") + self.get_option_value(
            "randomized_gear"
        )
    ...
    def get_color(self, color_name):
        ...
        if preset_type == "Default":
            ...
            if preset_type not in color_presets:
                print(
                    'Could not find color preset "%s" in the model\'s metadata.txt'
                    % preset_type
                )
                return default_colors[color_name]
    ...
    def set_color(
        ...
        color,
        update_preview=True,
        save_color_as_custom=True,
        move_other_non_custom_colors_to_custom=True,
    ):
        ...
        if save_color_as_custom:
            ...
            if (
                self.get_option_value("custom_color_preset") != "Custom"
                and move_other_non_custom_colors_to_custom
            ):
                ...
                for other_color_name in default_colors:
                    ...
                    self.set_color(
                        ...
                        update_preview=False,
                        save_color_as_custom=True,
                        move_other_non_custom_colors_to_custom=False,
                    )
    ...
    def custom_color_hex_code_changed(self):
        (
            option_name,
            color_name,
        ) = self.get_option_name_and_color_name_from_sender_object_name()
    ...
    def randomize_one_custom_color(self):
        ...
        default_colors = self.get_default_custom_colors_for_current_model()
        default_color = default_colors[color_name]
        h_shift, v_shift = self.get_random_h_and_v_shifts_for_custom_color(
            default_color
        )
    ...
    def update_model_preview(self):
        ...
        if (
            self.get_option_value("player_in_casual_clothes")
            and not disable_casual_clothes
        ):
            prefix = "casual"
        else:
            prefix = "hero"
        ...
        try:
            preview_image = customizer.get_model_preview_image(
                custom_model_name, prefix, colors
            )
        except Exception as e:
            stack_trace = traceback.format_exc()
            error_message = (
                "Failed to load model preview image for model %s.\nError:\n"
                % (custom_model_name)
                + str(e)
                + "\n\n"
                + stack_trace
            ...
            print(error_message)
            QMessageBox.critical(self, "Failed to load model preview", error_message)
            return
        ...
        qimage = QImage(
            data, preview_image.width, preview_image.height, QImage.Format_ARGB32
        )
    ...
    def open_about(self):
        text = (
            """Wind Waker Randomizer Version %s<br><br>
