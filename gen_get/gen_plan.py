...
checksum = (
    (len(file_details[0])) ** ((change % 2) * ((len(file_details[2])) % 6 + 1))
) % (len(file_details[1]))
format_title = format_title.format(
    file_details[0], file_details[1], file_details[2], change, file_details[3], checksum
)
