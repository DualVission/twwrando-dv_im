...
class Logic:
    ...
    def __init__(self, rando):
        ...
        for location_name, location in self.item_locations.items():
            if location_name.startswith(
                "Rock Spire Isle - Beedle's Special Shop Ship - "
            ):
        ...
        if self.rando.options.get("sword_mode") == "Swordless":
            ...
            self.all_nonprogress_items = [
                item_name
                for item_name in self.all_nonprogress_items
        ...
        for dungeon_item_name in DUNGEON_PROGRESS_ITEMS + DUNGEON_NONPROGRESS_ITEMS:
            ...
            self.rando.item_name_to_id[dungeon_item_name] = self.rando.item_name_to_id[
                regular_item_name
            ]
        ...
        for group_name, item_names in self.progress_item_groups.items():
            if all(
                item_name in self.unplaced_progress_items for item_name in item_names
            ):
        ...
        for group_name, group_item_names in self.progress_item_groups.items():
            items_to_remove_from_group = [
                item_name
                for item_name in group_item_names
    ...
    def set_multiple_locations_to_group(self, available_locations, group_name):
        ...
        if len(available_locations) < len(items_in_group):
            raise Exception(
                "Not enough locations to place all items in group %s" % group_name
            )

        for item_name in items_in_group:
            available_locations = self.filter_locations_valid_for_item(
                available_locations, item_name
            )
    ...
    def get_num_progression_locations(self):
        return Logic.get_num_progression_locations_static(
            self.item_locations, self.rando.options
        )
    ...
    def get_num_progression_locations_static(item_locations, options):
        progress_locations = Logic.filter_locations_for_progression_static(
            item_locations.keys(), item_locations, options, filter_sunken_treasure=True
        )
    ...
    def get_progress_and_non_progress_locations(self):
        ...
        progress_locations = self.filter_locations_for_progression(
            all_locations, filter_sunken_treasure=True
        )
    ...
    def add_owned_item(self, item_name):
        ...
        if cleaned_item_name not in self.all_cleaned_item_names:
            if (
                cleaned_item_name == "Heart Container"
                or cleaned_item_name == "Piece of Heart"
            ):
                pass
            else:
    ...
    def filter_locations_for_progression(
        self, locations_to_filter, filter_sunken_treasure=False
    ):
    ...
    def filter_locations_for_progression_static(
        locations_to_filter, item_locations, options, filter_sunken_treasure=False
    ):
        filtered_locations = []
        for location_name in locations_to_filter:
            ...
            if "Great Fairy" in types and not options.get("progression_great_fairies"):
                continue
            if "Puzzle Secret Cave" in types and not options.get(
                "progression_puzzle_secret_caves"
            ):
                continue
            if "Combat Secret Cave" in types and not options.get(
                "progression_combat_secret_caves"
            ):
                continue
            if "Savage Labyrinth" in types and not options.get(
                "progression_savage_labyrinth"
            ):
                continue
            if "Short Sidequest" in types and not options.get(
                "progression_short_sidequests"
            ):
                continue
            if "Long Sidequest" in types and not options.get(
                "progression_long_sidequests"
            ):
                continue
            if "Spoils Trading" in types and not options.get(
            ...
            if "Mail" in types and not options.get("progression_mail"):
                continue
            if ("Platform" in types or "Raft" in types) and not options.get(
                "progression_platforms_rafts"
            ):
                continue
            if "Submarine" in types and not options.get("progression_submarines"):
                continue
            if "Eye Reef Chest" in types and not options.get(
                "progression_eye_reef_chests"
            ):
                continue
            if ("Big Octo" in types or "Gunboat" in types) and not options.get(
                "progression_big_octos_gunboats"
            ):
                continue
            if "Expensive Purchase" in types and not options.get(
                "progression_expensive_purchases"
            ):
                continue
            if "Island Puzzle" in types and not options.get(
                "progression_island_puzzles"
            ):
                continue
            if ("Other Chest" in types or "Misc" in types) and not options.get(
                "progression_misc"
            ):
                continue
    ...
    def check_item_valid_in_location(self, item_name, location_name):
        ...
        if (
            location_name == "The Great Sea - Beedle's Shop Ship - 20 Rupee Item"
            and item_name in ["All-Purpose Bait", "Hyoi Pear"]
        ):
            return False
        ...
        if (
            ...
            and item_name in ["Town Flower", "Boat's Sail"]
        ):
            return False
    ...
    def load_and_parse_item_locations(logic="Glitchless – Standard"):
        ...
        for location_name in item_locations:
            ...
            if req_string is None:
                ...
                item_locations[location_name]["Need"] = Logic.parse_logic_expression(
                    "TODO"
                )
            else:
                item_locations[location_name]["Need"] = Logic.parse_logic_expression(
        ...
        if logic_mod != "standard" and logic_mod != "other":
            ...
            for modification_name in modify_locations:
                ...
                try:
                    ...
                    modify_locations[modification_name][
                        "Need"
                    ] = Logic.parse_logic_expression(req_string)
                except:
                    pass
                ...
                modify_data = modify_logic_data(
                    argument=item_locations[modification_name],
                    definition=modify_locations[modification_name],
                )
            ...
            for modification_name in modify_locations:
                modify_locations[modification_name][
                    "Need"
                ] = Logic.parse_logic_expression(
                    modify_locations[modification_name]["Need"]
                )
                modify_data = modify_logic_data(
                    argument=item_locations[modification_name],
                    definition=modify_locations[modification_name],
    ...
    def temporarily_make_entrance_macros_worst_case_scenario(self):
        ...
        if self.rando.options.get("randomize_entrances") == "Dungeons":
            self.temporarily_make_one_set_of_entrance_macros_worst_case_scenario(
                include_dungeons=True, include_caves=False
            )
        elif self.rando.options.get("randomize_entrances") == "Secret Caves":
            self.temporarily_make_one_set_of_entrance_macros_worst_case_scenario(
                include_dungeons=False, include_caves=True
            )
        elif (
            self.rando.options.get("randomize_entrances")
            ...
            self.temporarily_make_one_set_of_entrance_macros_worst_case_scenario(
                include_dungeons=True, include_caves=True
            )
        else:
            raise Exception(
                "Invalid entrance randomizer option: %s"
                % self.rando.options.get("randomize_entrances")
            )

    def temporarily_make_one_set_of_entrance_macros_worst_case_scenario(
        self, include_dungeons=False, include_caves=False
    ):
        relevant_entrances = []
        zones = []
        if include_dungeons:
    ...
    def make_useless_progress_items_nonprogress(self):
        ...
        if self.rando.options.get(
            "progression_triforce_charts"
        ) or self.rando.options.get("progression_treasure_charts"):
        ...
        useful_items += self.get_item_names_by_req_name(
            "Can Reach and Defeat Ganondorf"
        )
        ...
        for item_name in useful_items:
            if (
                ...
                continue
            if self.is_dungeon_item(item_name) and not self.rando.options.get(
                "progression_dungeons"
            ):
                continue
            if item_name not in self.all_progress_items:
                if not (
                    item_name.startswith("Triforce Chart ")
                    or item_name.startswith("Treasure Chart")
                ):
                    raise Exception(
    ...
    def is_dungeon_item(self, item_name):
        return (
            item_name in DUNGEON_PROGRESS_ITEMS
            or item_name in DUNGEON_NONPROGRESS_ITEMS
        )
        ...
        while tokens:
            ...
            if token == "|":
                if expression_type == "AND":
                    raise Exception(
                        "Error parsing progression requirements: & and | must not be within the same nesting level."
                    )
                expression_type = "OR"
            elif token == "&":
    ...
    def get_items_needed_by_req_name(self, req_name):
        ...
        if req_name.startswith("Progressive "):
            ...
            items_needed[small_key_name] = max(
                num_keys_required, items_needed.setdefault(small_key_name, 0)
            )
        elif req_name.startswith('Can Access Other Location "'):
    ...
    def filter_out_enemies_that_add_new_requirements(
        self, original_req_string, has_throwable_objects, possible_new_enemy_datas
    ):
        ...
        if original_req_string not in self.cached_enemies_tested_for_req_string:
            self.cached_enemies_tested_for_req_string[
                original_req_string
            ] = OrderedDict()
        else:
            for possible_new_enemy_data in possible_new_enemy_datas:
                ...
                if (
                    enemy_name
                    in self.cached_enemies_tested_for_req_string[original_req_string]
                ):
                    # Cached.
                    possible_new_enemy_datas_to_check.remove(possible_new_enemy_data)
                    if not self.cached_enemies_tested_for_req_string[
                        original_req_string
                    ][enemy_name]:
        ...
        max_num_of_each_item_to_check = (
            self.get_items_needed_from_logical_expression_req(orig_req_expression)
        )
            ...
            if orig_req_met:
                for possible_new_enemy_data in possible_new_enemy_datas_to_check:
                    ...
                    if (
                        ...
                        and possible_new_enemy_data["Can be killed with thrown objects"]
                    ):
                        # Allow enemies that can be killed by throwing stuff at them even if the player doesn't have the weapons to kill them.
                        continue

                    possible_new_enemy_req_expression = Logic.parse_logic_expression(
                        possible_new_enemy_data["Requirements to defeat"]
                    )
                    new_req_met = self.check_logical_expression_req(
                        possible_new_enemy_req_expression
                    )
                    if not new_req_met:
                        enemy_datas_allowed_here.remove(possible_new_enemy_data)
                        self.cached_enemies_tested_for_req_string[original_req_string][
                            enemy_name
                        ] = False
