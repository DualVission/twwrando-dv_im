    ...
    for key in valid_keys:
        try:
            if mode == "o":
                ...
                print(
                    "\n{} formating is incorrect for:\n\n{}\n{}:\t{}\n\nPlease correct this before a seed can be generated as intended.".format(
                        run, definition, key, definition[key]
                    )
                )
        except:
            pass
...
def change_ship_starting_island(self, starting_island_room_index):
    island_dzx = self.get_arc(
        "files/res/Stage/sea/Room%d.arc" % starting_island_room_index
    ).get_file("room.dzr")
...
def fix_deku_leaf_model(self):
    ...
    deku_leaf_actors = [
        actor for actor in dzx.entries_by_type("ACTR") if actor.name == "itemDek"
    ]
...
def allow_all_items_to_be_field_items(self):
    ...
    for item_id in self.item_ids_without_a_field_model:
        ...
        item_resources_addr_to_copy_from = (
            item_resources_list_start + item_id_to_copy_from * 0x24
        )
        ...
        if item_id == 0xAA:
            ...
            arc_name_pointer = self.main_custom_symbols[
        ...
        data1 = self.dol.read_data(
            read_bytes, item_resources_addr_to_copy_from + 8, 0xD
        )
        data2 = self.dol.read_data(
            read_bytes, item_resources_addr_to_copy_from + 0x1C, 4
        )
    ...
    for item_id in range(0, 0x83 + 1):
        ...
        location_of_items_switch_statement_case = (
            itemGetExecute_switch_statement_entries_list_start + item_id * 4
        )
        original_switch_case = self.dol.read_data(
            read_u32, location_of_items_switch_statement_case
        )
        if original_switch_case == 0x800F6C8C:
            self.dol.write_data(
                write_u32, location_of_items_switch_statement_case, 0x800F675C
            )
    ...
    for item_id in range(0x20, 0x44 + 1):
        ...
        location_of_items_switch_statement_case = (
            mode_wait_switch_statement_entries_list_start + item_id * 4
        )
        self.dol.write_data(
            write_u32, location_of_items_switch_statement_case, 0x800F8160
        )
...
def remove_shop_item_forced_uniqueness_bit(self):
    ...
    for shop_item_index in [
        0,
        0xB,
        0xC,
        0xD,
    ]:  # Bait Bag, Empty Bottle, Piece of Heart, and Treasure Chart 4 in Beedle's shops
        shop_item_data_addr = shop_item_data_list_start + shop_item_index * 0x10
        buy_requirements_bitfield = self.dol.read_data(
            read_u8, shop_item_data_addr + 0xC
        )
        buy_requirements_bitfield = buy_requirements_bitfield & (
            ~2
        )  # Bit 02 specifies that the player must not already own this item
        self.dol.write_data(
            write_u8, shop_item_data_addr + 0xC, buy_requirements_bitfield
        )
...
def remove_forsaken_fortress_2_cutscenes(self):
    ...
    exit = next(
        (
            exit
            for exit in dzx.entries_by_type("SCLS")
            if exit.dest_stage_name == "M2ganon"
        ),
        None,
    )
    if exit:
...
def make_items_progressive(self):
    ...
    for sword_item_id in [0x38, 0x39, 0x3A, 0x3D, 0x3E]:
        ...
        self.dol.write_data(
            write_u32,
            sword_item_get_func_addr,
            self.main_custom_symbols["progressive_sword_item_func"],
        )
    ...
    for bow_item_id in [0x27, 0x35, 0x36]:
        ...
        self.dol.write_data(
            write_u32,
            bow_item_get_func_addr,
            self.main_custom_symbols["progressive_bow_func"],
        )
    ...
    for wallet_item_id in [0xAB, 0xAC]:
        ...
        self.dol.write_data(
            write_u32,
            wallet_item_get_func_addr,
            self.main_custom_symbols["progressive_wallet_item_func"],
        )
    ...
    for bomb_bag_item_id in [0xAD, 0xAE]:
        ...
        self.dol.write_data(
            write_u32,
            bomb_bag_item_get_func_addr,
            self.main_custom_symbols["progressive_bomb_bag_item_func"],
        )
    ...
    for quiver_item_id in [0xAF, 0xB0]:
        ...
        self.dol.write_data(
            write_u32,
            quiver_item_get_func_addr,
            self.main_custom_symbols["progressive_quiver_item_func"],
        )
    ...
    for picto_box_item_id in [0x23, 0x26]:
        ...
        self.dol.write_data(
            write_u32,
            picto_box_item_get_func_addr,
            self.main_custom_symbols["progressive_picto_box_item_func"],
        )
  ...
  """
    ...
    self.dol.write_data(
        write_u32, 0x800C3768, 0x60000000
    )  # Don't set current magic meter
...
def make_sail_behave_like_swift_sail(self):
    ...
    new_sail_tex_image_path = os.path.join(
        ASSETS_PATH, ("{} texture.png".format(sailDict[sail_color]["Preface"]))
    )
    new_sail_icon_image_path = os.path.join(
        ASSETS_PATH, ("{} icon.png".format(sailDict[sail_color]["Preface"]))
    )
    new_sail_itemget_tex_image_path = os.path.join(
        ASSETS_PATH, ("{} item get texture.png".format(sailDict[sail_color]["Preface"]))
    )
...
def add_more_magic_jars(self):
    ...
    drc_center_room = self.get_arc("files/res/Stage/M_NewD2/Room2.arc").get_file(
        "room.dzr"
    )
    ...
    drc_before_boss_room = self.get_arc("files/res/Stage/M_NewD2/Room10.arc").get_file(
        "room.dzr"
    )
    ...
    grass1.grass_item_drop_type = (
        0x38  # 62.50% chance of small magic, 37.50% chance of large magic
    )
    grass1.x_pos = 209694
    grass1.y_pos = 1900
    grass1.z_pos = -202463
    ...
    totg_before_miniboss_room = self.get_arc(
        "files/res/Stage/Siren/Room14.arc"
    ).get_file("room.dzr")
...
def update_game_name_icon_and_banners(self):
    ...
    image_data, _, _, image_width, image_height = texture_utils.encode_image_from_path(
        new_image_file_path, image_format, palette_format
    )
...
def allow_dungeon_items_to_appear_anywhere(self):
    ...
    for short_dungeon_name, base_item_name, item_id in dungeon_items:
        ...
        self.dol.write_data(
            write_u32, item_get_func_addr, self.main_custom_symbols[custom_symbol_name]
        )
        ...
        if base_item_name == "Small Key":
            ...
            description = word_wrap_string(
                ...
                % (get_indefinite_article(dungeon_name), dungeon_name)
            )
        ...
        item_resources_addr_to_copy_from = (
            item_resources_list_start + base_item_id * 0x24
        )
        field_item_resources_addr_to_copy_from = (
            field_item_resources_list_start + base_item_id * 0x24
        )
        ...
        self.dol.write_data(
            write_u32, item_resources_addr + 4, item_icon_filename_pointer
        )
        ...
        data3 = self.dol.read_data(
            read_bytes, item_resources_addr_to_copy_from + 0x15, 7
        )
        self.dol.write_data(write_bytes, item_resources_addr + 0x15, data3)
        data4 = self.dol.read_data(
            read_bytes, item_resources_addr_to_copy_from + 0x20, 4
        )
        self.dol.write_data(write_bytes, item_resources_addr + 0x20, data4)

        data5 = self.dol.read_data(
            read_bytes, field_item_resources_addr_to_copy_from + 0x11, 3
        )
        self.dol.write_data(write_bytes, field_item_resources_addr + 0x11, data5)
        data6 = self.dol.read_data(
            read_bytes, field_item_resources_addr_to_copy_from + 0x18, 4
        )
...
def remove_ballad_of_gales_warp_in_cutscene(self):
    for island_index in range(1, 49 + 1):
        dzx = self.get_arc("files/res/Stage/sea/Room%d.arc" % island_index).get_file(
            "room.dzr"
        )
        for spawn in dzx.entries_by_type("PLYR"):
            if spawn.spawn_type == 9:  # Spawn type is warping in on a cyclone
                spawn.spawn_type = (
                    2  # Change to spawn type of instantly spawning on KoRL instead
                )
                spawn.save_changes()
...
def update_shop_item_descriptions(self):
    ...
    msg.string = "\\{1A 06 FF 00 00 01}%s  %d Rupees\\{1A 06 FF 00 00 00}" % (
        item_name,
        cost,
    )
    ...
    msg.string = (
        "\\{1A 06 FF 00 00 01}%s  %d Rupees\n\\{1A 06 FF 00 00 00}This is my last one."
        % (item_name, cost)
    )
    msg = self.bmg.messages_by_id[12109]
    msg.string = (
        "This \\{1A 06 FF 00 00 01}%s\\{1A 06 FF 00 00 00} is a mere \\{1A 06 FF 00 00 01}%d Rupees\\{1A 06 FF 00 00 00}!\nBuy it! Buy it! Buy buy buy!\n\\{1A 05 00 00 08}I'll buy it\nNo thanks"
        % (item_name, cost)
    )
    ...
    msg.string = (
        "This \\{1A 06 FF 00 00 01}%s\\{1A 06 FF 00 00 00} is only \\{1A 06 FF 00 00 01}%d Rupees\\{1A 06 FF 00 00 00}!\nBuy it! Buy it! Buy buy buy!\n\\{1A 05 00 00 08}I'll buy it\nNo thanks"
        % (item_name, cost)
    )
    ...
    msg.string = (
        "This \\{1A 06 FF 00 00 01}%s \\{1A 06 FF 00 00 00}is just \\{1A 06 FF 00 00 01}%d Rupees!\\{1A 06 FF 00 00 00}\nBuy it! Buy it! Buy buy buy!\n\\{1A 05 00 00 08}I'll buy it\nNo thanks"
        % (item_name, cost)
    )
...
def update_battlesquid_item_names(self):
    ...
    msg.string += word_wrap_string(
        "Please take this \\{1A 06 FF 00 00 01}%s\\{1A 06 FF 00 00 00} as a sign of our gratitude. You are soooooo GREAT!"
        % item_name,
        max_line_length=43,
    )
    ...
    msg.string += word_wrap_string(
        "This is our thanks to you! It's been passed down on our island for many years, so don't tell the island elder, OK? Here...\\{1A 06 FF 00 00 01}\\{1A 05 00 00 39} \\{1A 06 FF 00 00 00}Please accept this \\{1A 06 FF 00 00 01}%s\\{1A 06 FF 00 00 00}!"
        % item_name,
        max_line_length=43,
    )
...
def update_item_names_in_letter_advertising_rock_spire_shop(self):
    item_name_1 = self.logic.done_item_locations[
        "Rock Spire Isle - Beedle's Special Shop Ship - 500 Rupee Item"
    ]
    item_name_2 = self.logic.done_item_locations[
        "Rock Spire Isle - Beedle's Special Shop Ship - 950 Rupee Item"
    ]
    item_name_3 = self.logic.done_item_locations[
        "Rock Spire Isle - Beedle's Special Shop Ship - 900 Rupee Item"
    ]
    ...
    hint_string = "Do you have need of {} \\{{1A 06 FF 00 00 01}}{}\\{{1A 06 FF 00 00 00}}, {} \\{{1A 06 FF 00 00 01}}{}\\{{1A 06 FF 00 00 00}}, or {} \\{{1A 06 FF 00 00 01}}{}\\{{1A 06 FF 00 00 00}}? We have them at special bargain prices.".format(
        get_indefinite_article(item_name_1),
        item_name_1,
        get_indefinite_article(item_name_2),
        item_name_2,
        get_indefinite_article(item_name_3),
        item_name_3,
    )
...
def update_savage_labyrinth_hint_tablet(self):
    ...
    floor_30_item_name = self.logic.done_item_locations[
        "Outset Island - Savage Labyrinth - Floor 30"
    ]
    floor_50_item_name = self.logic.done_item_locations[
    ...
    if floor_30_is_progress and not floor_30_item_name in self.progress_item_hints:
        raise Exception(
            "Could not find progress item hint for item: %s" % floor_30_item_name
        )
    if floor_50_is_progress and not floor_50_item_name in self.progress_item_hints:
        raise Exception(
            "Could not find progress item hint for item: %s" % floor_50_item_name
        )
...
def update_randomly_chosen_hints(self):
    ...
    while True:
        ...
        is_puzzle_cave = (
            "Puzzle Secret Cave" in self.logic.item_locations[location_name]["Types"]
        )
        is_combat_cave = (
            "Combat Secret Cave" in self.logic.item_locations[location_name]["Types"]
        )
        is_savage = (
            "Savage Labyrinth" in self.logic.item_locations[location_name]["Types"]
        )
        if zone_name in self.dungeon_and_cave_island_locations and (
        ...
        if (
            item_name,
            island_name,
        ) in unique_items_given_hint_for:  # Don't give hint for same type of item in same zone
            continue
...
def update_fishmen_hints(self, hints):
    ...
    for fishman_hint_number in range(len(islands)):
        ...
        hint_lines.append(
            "Could be worth a try checking that place out. If you know where it is, of course."
        )
...
def update_big_octo_great_fairy_item_name_hint(self, hint):
    ...
    self.bmg.messages_by_id[12015].string = word_wrap_string(
        "\\{1A 06 FF 00 00 05}In \\{1A 06 FF 00 00 01}%s\\{1A 06 FF 00 00 05}, you will find an item."
        % island_hint_name,
        max_line_length=43,
    )
    self.bmg.messages_by_id[12016].string = word_wrap_string(
        "\\{1A 06 FF 00 00 05}...\\{1A 06 FF 00 00 01}%s\\{1A 06 FF 00 00 05} which may help you on your quest."
        % item_hint_name.capitalize(),
        max_line_length=43,
    )
...
def shorten_zephos_event(self):
    ...
    event_list = self.get_arc("files/res/Stage/sea/Stage.arc").get_file(
        "event_list.dat"
    )
...
def dungeon_number(self):
    ...
    if dungeon_num_race_mode == "NaN":  # If it is not changed from the default state
        ...
        if dungeon_num_race_mode == "Random":  # If it is Random
            ...
            dungeon_num_race_mode = int(
                dungeon_num_race_mode
...
def set_starting_health(self):
    ...
    starting_quarter_hearts_address = self.main_custom_symbols[
...
def add_pirate_ship_to_windfall(self):
    ...
    event_list = self.get_arc("files/res/Stage/Asoko/Stage.arc").get_file(
        "event_list.dat"
    )

    windfall_layer_2_actors = windfall_dzr.entries_by_type_and_layer("ACTR", 2)
    layer_2_pirate_ship = next(
        x for x in windfall_layer_2_actors if x.name == "Pirates"
    )
    ...
    isle_link_0_aw_index = (
        0x19  # The index of IsleLink_0.aw, the wave bank containing Aryll's voice.
    )

    asoko_bgm_info_ptr = stage_bgm_info_list_start + asoko_spot_id * 4
    new_second_scene_wave_ptr = (
        second_dynamic_scene_waves_list_start + new_second_scene_wave_index * 2
    )
    ...
    sw_op.first_switch_to_check = (
        countdown_not_happening_switch  # && inside_chest_room_switch
    )
    ...
    sw_op.num_switches_to_check = 2
    sw_op.first_switch_to_check = (
        countdown_happening_switch  # || aryll_opened_door_switch
    )
...
WarpPotData = namedtuple(
    "WarpPotData", "stage_name room_num x y z y_rot event_reg_index"
)
    ...
    for warp_pot_datas_in_this_cycle in INTER_DUNGEON_WARP_DATA:
        for warp_pot_index, warp_pot_data in enumerate(warp_pot_datas_in_this_cycle):
            room_arc_path = "files/res/Stage/%s/Room%d.arc" % (
                warp_pot_data.stage_name,
                warp_pot_data.room_num,
            )
            ...
            warp_pot.name = "Warpts%d" % (
                warp_pot_index + 1
            )  # Warpts1 Warpts2 or Warpts3
...
def increase_player_movement_speeds(self):
    ...
    self.dol.write_data(
        write_float, 0x8035D3D0, 6.0 / 17.0
    )  # Rolling speed multiplier on walking speed
...
def increase_block_moving_animation(self):
    ...
    for i in range(13):  # 13 types of blocks total.
        block_rel.write_data(
            write_u16, offset + 0x04, 12
        )  # Reduce number frames for pushing to last from 20 to 12
        block_rel.write_data(
            write_u16, offset + 0x0A, 12
        )  # Reduce number frames for pulling to last from 20 to 12
        offset += 0x9C
...
def change_starting_clothes(self):
    ...
    should_start_with_heros_clothes_address = self.main_custom_symbols[
...
def shorten_auction_intro_event(self):
    event_list = self.get_arc("files/res/Stage/Orichh/Stage.arc").get_file(
        "event_list.dat"
    )
...
def update_starting_gear(self):
    ...
    if len(starting_gear) > MAXIMUM_ADDITIONAL_STARTING_ITEMS:
        raise Exception(
            "Tried to start with more starting items than the maximum number that was allocated"
        )
    ...
    # Write end marker.
    self.dol.write_data(
        write_u8, starting_gear_array_address + len(starting_gear), 0xFF
    )
...
def add_hint_signs(self):
    ...
    bomb_flowers = [
        actor
        for actor in dzx.entries_by_type_and_layer("ACTR", None)
        if actor.name == "BFlower"
    ]
...
def add_chest_in_place_of_jabun_cutscene(self):
    ...
    jabun_chest.behavior_type = (
        5  # Necessary for the chest to bob up and down with the raft it's on
    )
    jabun_chest.x_pos = 0
    ...
    director = next(
        actor for actor in unlock_cave_event.actors if actor.name == "DIRECTOR"
    )
...
def add_chest_in_place_of_master_sword(self):
    ...
    ms_chamber_dzr = self.get_arc("files/res/Stage/kenroom/Room0.arc").get_file(
        "room.dzr"
    )
    ...
    ms_actors = [
        x
        for x in ms_chamber_dzr.entries_by_type_and_layer("ACTR", None)
        if x.name in ["VmsMS", "VmsDZ"]
    ]
    ...
    layer_5_actors_to_copy = [
        x for x in layer_5_actors if x.name in ["Tn", "ALLdie", "Yswdr00"]
    ]
    ...
    ms_chest.appear_condition_switch = (
        5  # The Mighty Darknuts set switch 5 when they die.
    )
    ...
    spawn = next(
        spawn
        for spawn in ms_chamber_dzr.entries_by_type("PLYR")
        if spawn.spawn_id == 10
    )
    spawn.y_pos = -2949.39
    spawn.z_pos = -4240.7
...
def remove_phantom_ganon_requirement_from_eye_reefs(self):
    ...
    for island_number in [24, 46, 22, 8, 37, 25]:
        eye_reef_dzr = self.get_arc(
            "files/res/Stage/sea/Room%d.arc" % island_number
        ).get_file("room.dzr")
        actors = eye_reef_dzr.entries_by_type("ACTR")
        cannons = [x for x in actors if x.name == "Ocanon"]
        for cannon in cannons:
            if (
                cannon.enable_spawn_switch == 0x2A
            ):  # Switch 2A is Phantom Ganon being dead.
                cannon.enable_spawn_switch = 0xFF
                cannon.save_changes()
        gunboats = [x for x in actors if x.name == "Oship"]
        for gunboat in gunboats:
            if (
                gunboat.aux_params_1 & 0xFF
            ) == 0x2A:  # Switch 2A is Phantom Ganon being dead.
...
def add_new_bog_warp(self):
    ...
    (
        high_halfword,
        low_halfword,
    ) = patcher.split_pointer_into_high_and_low_half_for_hardcoding(
        custom_warp_table_address
    )
    ...
    custom_warp_float_bank_address = self.main_custom_symbols[
    ...
    self.dol.write_data(
        write_u16, 0x801B7C3C + 2, 0
    )  # Reading X positions in dMenu_Fmap_c::init_warpMode
    self.dol.write_data(
        write_u16, 0x801B7C44 + 2, new_num_warps * 4
    )  # Reading Y positions in dMenu_Fmap_c::init_warpMode
    self.dol.write_data(
        write_u16, 0x801B9378 + 2, 0
    )  # Reading X positions in dMenu_Fmap_c::warpAreaAnime0
    self.dol.write_data(
        write_u16, 0x801B9380 + 2, new_num_warps * 4
    )  # Reading Y positions in dMenu_Fmap_c::warpAreaAnime0
    self.dol.write_data(
        write_u16, 0x801B93A4 + 2, new_num_warps * 2 * 4
    )  # Reading unknown value in dMenu_Fmap_c::warpAreaAnime0
    self.dol.write_data(
        write_u16, 0x801B93C8 + 2, new_num_warps * 2 * 4 + 4
    )  # Reading unknown value in dMenu_Fmap_c::warpAreaAnime0

    # These handle displaying the spinning warp icons on the warp select screen.
    self.dol.write_data(
        write_u16, 0x801B7988 + 2, new_num_warps
    )  # dMenu_Fmap_c::_open_warpMode
    self.dol.write_data(
        write_u16, 0x801B7C80 + 2, new_num_warps
    )  # dMenu_Fmap_c::init_warpMode
...
def enable_heap_display(self):
    ...
    self.dol.write_data(
        write_u8, 0x800063E7, 1
    )  # Hardcoded default value for mDisplayHeapSize (in func main01)
    ...
    self.dol.write_data(
        write_u32, 0x800084A0, 0x60000000
    )  # nop (in mDoGph_AfterOfDraw)
...
def add_failsafe_id_0_spawns(self):
    ...
    for stage_name, room_number, spawn_id_to_copy in spawns_to_copy:
        dzr = self.get_arc(
            "files/res/Stage/%s/Room%d.arc" % (stage_name, room_number)
        ).get_file("room.dzr")
        spawns = dzr.entries_by_type("PLYR")
        spawn_to_copy = next(
            spawn for spawn in spawns if spawn.spawn_id == spawn_id_to_copy
        )
