            ...
            if file_path == "sys/main.dol":
                ...
                if offset >= free_space_start:
                    apply_free_space_patchlet_to_rel(
                        self, file_path, offset, new_bytes, relocations
                    )
                else:
                    rel.write_data(
...
def apply_free_space_patchlet_to_rel(self, file_path, offset, new_bytes, relocations):
    ...
    if relocations:
        add_relocations_to_rel(
            ...
            file_path,
            rel_section_index,
            patchlet_offset_into_curr_section,
            relocations,
        )
...
def add_relocations_to_rel(
    self, file_path, rel_section_index, offset_into_section, relocations
):
    ...
    for relocation_dict in relocations:
        ...
        if symbol_name in self.main_custom_symbols:
            ...
            if symbol_name not in other_rel_symbols:
                raise Exception(
                    f'Symbol "{symbol_name}" could not be found in the symbol map for REL {rel_name}.rel'
                )
            ...
            (
                other_rel_section_index,
                relative_offset,
            ) = other_rel.convert_rel_offset_to_section_index_and_relative_offset(
