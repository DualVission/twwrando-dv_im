    ...
    if branch_label_match:
        ...
        if elf_relocation.type == ELFRelocationType.R_PPC_REL24:
            if (
                ...
                raise Exception(
                    f"Relocation failed: Cannot branch from {branch_src_offset:X} to {branch_dest_offset:X} with a 24-bit relative offset."
                )
            ...
            if relative_branch_offset > 0x7FFF or relative_branch_offset < -0x8000:
                raise Exception(
                    f"Relocation failed: Cannot branch from {branch_src_offset:X} to {branch_dest_offset:X} with a 14-bit relative offset."
                )
        ...
        for line in asm.splitlines():
            ...
            if open_file_match:
                ...
                if most_recent_file_path or most_recent_org_offset is not None:
                    raise Exception(
                        f"File {most_recent_file_path} was not closed before opening new file {relative_file_path}"
                    )
                ...
                ] += f"{branch_temp_label} = 0x{branch_dest:X};\n"
    ...
    for patch_name, code_chunks_for_patch in code_chunks.items():
        ...
        for file_path, code_chunks_for_file in code_chunks_for_patch.items():
            ...
            if file_path != "sys/main.dol":
                ...
                for symbol_name, symbol_address in custom_symbols[
                    "sys/main.dol"
                ].items():
                    temp_linker_script += f"{symbol_name} = 0x{symbol_address:08X};\n"
                ...
                with open(map_name) as f:
                    ...
                    for line in f.read().splitlines():
                        ...
                        if on_custom_symbols:
                            ...
                            temp_linker_script += (
                                f"{symbol_name} = 0x{symbol_address:08X};\n"
                            )
