...
def disassemble_all_code(self):
    ...
    for file_path_in_gcm in files_to_disassemble:
        ...
        is_rel = file_ext == ".rel"
        if is_rel:
            add_relocations_and_symbols_to_rel(
                asm_path,
                bin_path,
                file_path_in_gcm,
                main_symbols,
    ...
    for module_num, relocation_entries in rel.relocation_entries_for_module.items():
        for relocation_data_entry in relocation_entries:
            ...
            replacement_location = (
                curr_section_offset + relocation_data_entry.relocation_offset
            )
            ...
            if module_num == 0:
                ...
                replacements[
                    rounded_down_location
                ] = f"{relocation_data_entry.symbol_address:X}  {symbol_name}"
            else:
                if module_num == rel.id:
                    ...
                    section_offset_to_relocate_against = (
                        section_to_relocate_against.offset
                    )
                    ...
                    other_rel, other_rel_path_in_gcm = find_rel_by_module_num(
                        all_rels_by_path, module_num
                    )
                    other_rel_symbol_names = all_rel_symbols_by_path[
                        other_rel_path_in_gcm
                    ]
                    other_rel_name = os.path.basename(other_rel_path_in_gcm)

                    section_to_relocate_against = other_rel.sections[
                        relocation_data_entry.section_num_to_relocate_against
                    ]
                    section_offset_to_relocate_against = (
                        section_to_relocate_against.offset
                    ...
                    if (
                        other_rel.bss_section_index
                        and relocated_offset >= other_rel.bss_offset
                    ):
    ...
    for line in asm.splitlines():
        ...
        if match:
            ...
            if not check_offset_in_executable_rel_section(word_offset, rel):
                ...
                before_disassembly_match = re.search(
                    r"^( +[0-9a-f]+:\s(?:[0-9a-f]{2} ){4}).+", line
                )
        ...
        if match:
            ...
            if offset in replacements:
                ...
                branch_match = re.search(
                    r"\s(bl|b|beq|bne|blt|bgt|ble|bge|bdnz|bdz)\s+0x([0-9a-f]+)",
                    line,
                    re.IGNORECASE,
                )
                if branch_match:
                    ...
                    if branch_offset in rel_symbol_names:
                        ...
                        out_str += (
                            get_padded_comment_string_for_line(line) + symbol_name
                        )
        ...
        while True:
            ...
            if match:
                ...
                if not check_offset_in_executable_dol_section(self, offset):
                    # Remove the disassembled code for non-executable sections since it will be nonsense, not actually code.
                    before_disassembly_match = re.search(
                        r"^( *[0-9a-f]+:\s((?:[0-9a-f]{2} ){4})).+", line, re.IGNORECASE
                    )
                    if before_disassembly_match:
                        line = before_disassembly_match.group(1)
            ...
            branch_match = re.search(
                r"^(.+ \t(?:bl|b|beq|bne|blt|bgt|ble|bge|bdnz|bdz)\s+0x)([0-9a-f]+)$",
                line,
                re.IGNORECASE,
            )
            addi_match = re.search(
                r"^.+ \t(?:addi)\s+r\d+,(r\d+),(-?\d+)$", line, re.IGNORECASE
            )
            load_or_store_match = re.search(
                r"^.+ \t(?:"
                + "|".join(ALL_LOAD_OR_STORE_OPCODES)
                + r")\s+[rf]\d+,(-?\d+)\((r\d+)\)$",
                line,
                re.IGNORECASE,
            )
            if branch_match:
                ...
                if source_register == "r2":
                    ...
                    (
                        last_line_address,
                        last_line_symbol_name,
                        last_line_source_register,
                    ) = last_line_loaded_symbol_info
                ...
                if address is not None:
                    ...
                    loaded_string = self.dol.read_data(
                        read_str_until_null_character, loaded_string_address
                    )
            ...
            lis_match = re.search(
                r"^.+ \t(?:lis)\s+(r\d+),(-?\d+)$", line, re.IGNORECASE
            )
            if lis_match:
...
def get_main_symbols(framework_map_contents):
    main_symbols = {}
    matches = re.findall(
        r"^  [0-9a-f]{8} [0-9a-f]{6} ([0-9a-f]{8})(?: +\d+)? (.+?) \t",
    ...
    for line in rel_map_lines:
        ...
        if found_memory_map:
            section_match = re.search(
                r"^ +\.(text|ctors|dtors|rodata|data|bss)  [0-9a-f]{8} ([0-9a-f]{8}) [0-9a-f]{8}$",
                line,
            )
            if section_match:
    ...
    for line in rel_map_lines:
        ...
        if section_header_match:
            ...
            if current_section_name in section_name_to_section_index:
                current_section_index = section_name_to_section_index[
...
def get_extra_comment_for_asm_line(line):
    ...
    rlwinm_match = re.search(
        r"^.+ \t(?:rlwinm\.?)\s+(r\d+),(r\d+),(\d+),(\d+),(\d+)$", line, re.IGNORECASE
    )
    clrlwi_match = re.search(
        r"^.+ \t(?:clrlwi\.?)\s+(r\d+),(r\d+),(\d+)$", line, re.IGNORECASE
    )
    rotlwi_match = re.search(
        r"^.+ \t(?:rotlwi\.?)\s+(r\d+),(r\d+),(\d+)$", line, re.IGNORECASE
    )

    rlwimi_match = re.search(
        r"^.+ \t(?:rlwimi\.?)\s+(r\d+),(r\d+),(\d+),(\d+),(\d+)$", line, re.IGNORECASE
    )
    ...
    if rlwinm_match or clrlwi_match or rotlwi_match or rlwimi_match:
        ...
        if rlwimi_match:
            ...
            if l_shift == 0:
                ...
                comment += "{} = ({} & 0x{:08X}) >> 0x{:02X}".format(
                    dst_reg, src_reg, adjusted_mask, -l_shift
                )
            else:
