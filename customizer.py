...
MAX_ALLOWED_TOTAL_ARC_FILE_SIZE_SUM_INCREASE_IN_BYTES = (
    1525678 - ORIG_LINK_ARC_FILE_SIZE_IN_BYTES
)
    ...
    if custom_model_name == "Random":
        ...
        try:
            ...
            old_format_match = re.search(
                r"^ +\S[^:]*: +[0-9A-F]{6}$", metadata_str, re.IGNORECASE | re.MULTILINE
            )
        ...
        metadata["mouth_color_mask_paths"].append(
            None
        )  # Dummy entry for mouth number 0, which doesn't exist
        ...
        for key, value in metadata.items():
            if key in ["hero_custom_colors", "casual_custom_colors"]:
                ...
                for custom_color_name, hex_color in value.items():
                    try:
                        value[custom_color_name] = parse_hex_color(
                            hex_color, use_old_color_format
                        )
                    except InvalidColorError as e:
                        error_message = 'Custom color "{}" has an invalid base color specified in metadata.txt: "{}"'.format(
                            custom_color_name, repr(hex_color)
                        )
                        return {
                            "error_message": error_message,
                        }
                    ...
                    mask_path = os.path.join(
                        color_masks_path, f"{prefix}_{custom_color_name}.png"
                    )
                    metadata["%s_color_mask_paths" % prefix][
                        custom_color_name
                    ] = mask_path
                    hands_mask_path = os.path.join(
                        color_masks_path, f"hands_{prefix}_{custom_color_name}.png"
                    )
                    metadata["hands_%s_color_mask_paths" % prefix][
                        custom_color_name
                    ] = hands_mask_path
                    hitomi_mask_path = os.path.join(
                        color_masks_path, f"hitomi_{prefix}_{custom_color_name}.png"
                    )
                    metadata["hitomi_%s_color_mask_paths" % prefix][
                        custom_color_name
                    ] = hitomi_mask_path
                    preview_mask_path = os.path.join(
                        previews_path, f"preview_{prefix}_{custom_color_name}.png"
                    )
                    metadata["preview_%s_color_mask_paths" % prefix][
                        custom_color_name
                    ] = preview_mask_path

                    for i in range(1, 9 + 1):
                        mouth_mask_path = os.path.join(
                            color_masks_path,
                            "mouths",
                            "mouthS3TC.%d_%s.png" % (i, custom_color_name),
                        )
                        metadata["mouth_color_mask_paths"][i][
                            custom_color_name
                        ] = mouth_mask_path
            ...
            if key in ["hero_color_presets", "casual_color_presets"]:
                ...
                for preset_name, preset in value.items():
                    for custom_color_name, hex_color in preset.items():
                        try:
                            preset[custom_color_name] = parse_hex_color(
                                hex_color, use_old_color_format
                            )
                        except InvalidColorError as e:
                            error_message = 'Color preset "{}"\'s color "{}" has an invalid base color specified in metadata.txt: "{}"'.format(
                                preset_name, custom_color_name, repr(hex_color)
                            )
                            return {
                                "error_message": error_message,
                            }
...
def parse_hex_color_old_format(hex_color):
    ...
    match = re.search(
        r"^([0-9A-F]{2})([0-9A-F]{2})([0-9A-F]{2})$", hex_color_string, re.IGNORECASE
    )
    if match:
        r, g, b = (
...
def decide_on_link_model(self):
    ...
    if custom_model_name == "Random" or custom_model_name == "Random (exclude Link)":
        ...
        if not custom_model_names:
            raise Exception(
                "No custom models to randomly choose from in the /models folder."
            )

        if custom_model_name == "Random":
            custom_model_names.append(
                None
            )  # Dummy entry to represent not changing Link's model
...
def replace_link_model(self):
    ...
    check_changed_archives_over_filesize_limit(
        orig_sum_of_changed_arc_sizes, new_sum_of_changed_arc_sizes, checked_arc_names
    )

    def replace_animation_arc(
        self, anim_arc_name, orig_anim_arc_file_size, revert_totals_after=False
    ):
        ...
        if os.path.isfile(anim_arc_path):
            ...
            check_changed_archives_over_filesize_limit(
                orig_sum_of_changed_arc_sizes,
                new_sum_of_changed_arc_sizes,
                checked_arc_names,
            )
    ...
    replace_animation_arc(
        self, "LkD00.arc", ORIG_LKD00_ARC_FILE_SIZE_IN_BYTES, revert_totals_after=True
    )
    ...
    if os.path.isfile(ship_path) and boat_get != True:
        ...
        if (
            custom_sail_tex_data.read() != orig_sail_tex_data.read()
            or orig_sail_tex.image_format != custom_sail_tex.image_format
        ):
    ...
    if not self.options.get("disable_custom_player_voice"):
        ...
        if os.path.isfile(jaiinit_aaf_path) and os.path.isfile(voice_aw_path):
            ...
            if os.path.isfile(ganont_aw_path):
                ...
                self.replace_raw_file(
                    "files/Audiores/Banks/GanonT_0.aw", ganont_aw_data
                )
...
def revert_item_models_in_arc_to_original(orig_arc, custom_arc):
    ...
    for orig_file_entry in orig_arc.file_entries:
        basename, file_ext = os.path.splitext(orig_file_entry.name)
        if file_ext == ".bdl" and basename not in [
            "cl",
            "katsura",
            "hands",
            "yamu",
...
def check_changed_archives_over_filesize_limit(
    orig_sum_of_changed_arc_sizes, new_sum_of_changed_arc_sizes, checked_arc_names
):
    # Validate the filesize didn't increase enough to cause memory issues.
    max_sum_of_changed_arc_sizes = (
        orig_sum_of_changed_arc_sizes
        + MAX_ALLOWED_TOTAL_ARC_FILE_SIZE_SUM_INCREASE_IN_BYTES
    )
...
def change_player_clothes_color(self):
    ...
    for custom_color_basename, base_color in custom_colors.items():
        ...
        if (
            os.path.isfile(hitomi_mask_path)
            ...
            if os.path.isfile(hitomi_mask_path):
                ...
                hitomi_image = texture_utils.color_exchange(
                    hitomi_image, base_color, custom_color, mask_path=hitomi_mask_path
                )
            elif custom_color_basename == hitomi_color_name:
                hitomi_image = texture_utils.color_exchange(
                    hitomi_image, base_color, custom_color, ignore_bright=True
                )
        ...
        mask_path = custom_model_metadata[prefix + "_color_mask_paths"][
            custom_color_basename
        ]
        ...
        if not os.path.isfile(mask_path) and replaced_any_pupils_for_this_color:
            ...
            link_main_image = texture_utils.color_exchange(
                link_main_image, base_color, custom_color, mask_path=mask_path
            )
        replaced_any = True
        ...
        for i in range(1, 9 + 1):
            ...
            if (
                os.path.isfile(mouth_mask_path)
                or custom_color_basename == mouth_color_name
            ):
                if i not in all_mouth_textures:
                    all_mouth_textures[i] = link_main_model.tex1.textures_by_name[
                        "mouthS3TC.%d" % i
                    ]
        ...
        if os.path.isfile(hands_mask_path):
            ...
            hands_image = texture_utils.color_exchange(
                hands_image, base_color, custom_color, mask_path=hands_mask_path
            )
    ...
    for texture in link_main_textures:
        if (
            ...
            and is_casual
            and texture.image_format == ImageFormat.C4
        ):
            ...
            self.custom_model_name == "Link"
            and not is_casual
            and texture.image_format == ImageFormat.CMPR
        ):
...
def get_model_preview_image(custom_model_name, prefix, selected_colors):
    ...
    for custom_color_basename, base_color in custom_colors.items():
        ...
        mask_path = custom_model_metadata["preview_" + prefix + "_color_mask_paths"][
            custom_color_basename
        ]
        check_valid_mask_path(mask_path)
...
def check_valid_mask_path(mask_path):
    ...
    true_filename = next(
        filename
        for filename in files_in_mask_folder
        if filename.lower() == given_filename.lower()
    )

    if given_filename != true_filename:
        raise Exception(
            f"Color mask path's actual capitalization differs from the capitalization given in metadata.txt.\nGiven: {given_filename}, actual: {true_filename}"
        )
