...
if IS_RUNNING_FROM_SOURCE:
    ...
    if os.path.isfile(git_commit_head_file):
        ...
        if head_file_contents.startswith("ref: "):
            ...
            path_to_hash_file = os.path.join(
                RANDO_ROOT_PATH, ".git", relative_path_to_hash_file
            )
...
class Randomizer:
    def __init__(
        ...
        if not self.dry_run:
            if not os.path.isfile(clean_iso_path):
                raise InvalidCleanISOError(
                    "Clean WW ISO does not exist: %s" % clean_iso_path
                )
            ...
            try:
                self.chart_list = self.get_arc("files/res/Msg/fmapres.arc").get_file(
                    "cmapdat.bin"
                )
        ...
        if num_progress_locations < num_progress_items:
            error_message = (
                "Not enough progress locations to place all progress items.\n\n"
            )
            error_message += "Total progress items: %d\n" % num_progress_items
            error_message += (
                "Progress locations with current options: %d\n\n"
    ...
    def randomize(self):
        ...
        if not self.dry_run:
            ...
            while True:
                ...
                yield (
                    "Saving randomized ISO...",
                    options_completed + int(percentage_done * 9),
                )
        options_completed += 9
    ...
    def read_text_file_lists(self):
        ...
        with open(os.path.join(DATA_PATH, "item_names.txt")) as f:
            matches = re.findall(
                r"^([0-9a-f]{2}) - (.+)$", f.read(), re.IGNORECASE | re.MULTILINE
            )
        ...
        with open(os.path.join(DATA_PATH, "item_resource_arc_name_pointers.txt")) as f:
            matches = re.findall(
                r"^([0-9a-f]{2}) ([0-9a-f]{8}) ", f.read(), re.IGNORECASE | re.MULTILINE
            )
    ...
    def add_new_rel(
        self, rel_path, new_rel, section_index_of_actor_profile, offset_of_actor_profile
    ):
        if not rel_path.startswith("files/rels/"):
            raise Exception(
                "Cannot add a new REL to a folder besides files/rels/: " + rel_path
            )
        ...
        if new_rel.id in profile_list.relocation_entries_for_module:
            raise Exception(
                f"Cannot add a new REL with a unique ID that is already present in the profile list:\nREL ID: {new_rel.id:03X}\nNew REL path: {rel_path}"
            )
    ...
    def save_randomized_iso(self):
        ...
        if self.export_disc_to_folder:
            output_folder_path = os.path.join(
                self.randomized_output_folder, "WW Random %s" % self.seed
            )
            ...
            output_file_path = os.path.join(
                self.randomized_output_folder, "WW Random %s.iso" % self.seed
            )
        ...
        while logic.unplaced_progress_items:
            ...
            for group_name, item_names in logic.progress_item_groups.items():
                entire_group_is_owned = all(
                    item_name in logic.currently_owned_items for item_name in item_names
                )
                if (
                    entire_group_is_owned
                    and group_name in logic.unplaced_progress_items
                ):
    ...
    def write_non_spoiler_log(self):
        ...
        ) = self.logic.get_progress_and_non_progress_locations()
        ...
        for zone_name, locations_in_zone in zones.items():
            if not any(
                loc for (loc, _) in locations_in_zone if loc in progress_locations
            ):
        ...
        for zone_name, locations_in_zone in zones.items():
            if not any(
                loc for (loc, _) in locations_in_zone if loc in nonprogress_locations
            ):
    ...
    def write_spoiler_log(self):
        ...
        spoiler_log_output_path = os.path.join(
            self.randomized_output_folder, "WW Random %s - Spoiler Log.txt" % self.seed
        )
    ...
    def write_error_log(self, error_message):
        ...
        error_log_output_path = os.path.join(
            self.randomized_output_folder, "WW Random %s - Error Log.txt" % self.seed
        )
