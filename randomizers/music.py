...
def randomize_music(self):
    ...
    num_bgms = read_u16(
        aaf_data, sound_categories_offset + 6 + bgm_category_index * 4 + 0
    )
    first_bgm_sound_index = read_u16(
        aaf_data, sound_categories_offset + 6 + bgm_category_index * 4 + 2
    )
    ...
    def filter_invalid_replacements_for_location(
        ...
        if (
            orig_default_bgm_name != "defaultse.bms"
            and orig_default_bgm_name not in orig_bgm_names_for_location
        ):
        ...
        for orig_bgm_name in orig_bgm_names_for_location:
            ...
            if loc_needed_first_scene_wave is not None:
                for (
                    bgm_name,
                    bgm_needed_first_scene_wave,
                ) in FIRST_SCENE_WAVE_NEEDED_FOR_BGM.items():
            ...
            if loc_needed_second_scene_wave is not None:
                for (
                    bgm_name,
                    bgm_needed_second_scene_wave,
                ) in SECOND_SCENE_WAVE_NEEDED_FOR_BGM.items():
    ...
    for spot_index in range(1, 0x78 + 1):
        if spot_index <= 0:
            ...
            stage_name_address = self.dol.read_data(
                read_u32, 0x8039C5B8 + (spot_index - 1) * 4
            )
            stage_name = self.dol.read_data(
                read_str_until_null_character, stage_name_address
            )
    ...
    def check_bgm_conflicts_with_orig_bgm(bgm_name, orig_bgm_name):
        if bgm_name in FIRST_SCENE_WAVE_NEEDED_FOR_BGM:
            ...
            for stage_name in all_stage_names_that_use_bgm[orig_bgm_name]:
                ...
                if (
                    decided_first_wave is not None
                    and decided_first_wave != new_first_wave_index
                ):
                    return True
        ...
        if bgm_name in SECOND_SCENE_WAVE_NEEDED_FOR_BGM:
            ...
            for stage_name in all_stage_names_that_use_bgm[orig_bgm_name]:
                ...
                if (
                    decided_second_wave is not None
                    and decided_second_wave != new_second_wave_index
                ):
                    return True
    ...
    for bgm_index in range(num_bgms):
        sound_index = first_bgm_sound_index + bgm_index
        sound_info_bytes = read_bytes(
            aaf_data, first_sound_info_offset + sound_index * 0x10, 0x10
        )
    ...
    for orig_bgm_name, new_possible_bgm_names in bgm_replacements_that_work.items():
        ...
        new_possible_bgm_names = [
            bgm_name
            for bgm_name in new_possible_bgm_names
        ...
        print(f"{orig_bgm_name} -> {new_bgm_name}")

        sound_index = first_bgm_sound_index + orig_bgm_index
        new_sound_info_bytes = orig_sound_infos[new_bgm_index]
        write_bytes(
            aaf_data, first_sound_info_offset + sound_index * 0x10, new_sound_info_bytes
        )
        ...
        if new_bgm_name in FIRST_SCENE_WAVE_NEEDED_FOR_BGM:
            ...
            for stage_name in all_stage_names_that_use_bgm[orig_bgm_name]:
                ...
                if (
                    ...
                    raise Exception(
                        "Tried to assign two first scene waves to stage %s" % stage_name
                    )
            ...
            for island_num in all_island_nums_that_use_bgm[orig_bgm_name]:
                ...
                if (
                    ...
                    raise Exception(
                        "Tried to assign two first scene waves to island %d"
                        % island_num
                    )
        ...
        if new_bgm_name in SECOND_SCENE_WAVE_NEEDED_FOR_BGM:
            ...
            for stage_name in all_stage_names_that_use_bgm[orig_bgm_name]:
                ...
                if (
                    ...
                    raise Exception(
                        "Tried to assign two second scene waves to stage %s"
                        % stage_name
                    )
            ...
            for island_num in all_island_nums_that_use_bgm[orig_bgm_name]:
                ...
                if (
                    ...
                    raise Exception(
                        "Tried to assign two second scene waves to island %d"
                        % island_num
                    )
