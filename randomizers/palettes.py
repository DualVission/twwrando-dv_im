    ...
    for randomizable_file_group in self.palette_randomizable_files:
        ...
        if randomizable_file_group["Particle IDs"]:
            ...
            for i in range(255):
                ...
                particle_ids_for_enemy_in_jpc = [
                    particle_id
                    for particle_id in jpc.particles_by_id
        ...
        for rarc_data in randomizable_file_group["RARCs"]:
            ...
            for file_entry in rarc.file_entries:
                ...
                if hasattr(j3d_file, "tex1"):
                    shift_all_colors_in_tex1(
                        self, file_name, j3d_file, h_shift, v_shift
                    )
                ...
                if hasattr(j3d_file, "mat3"):
                    shift_all_colors_in_mat3(
                        self, file_name, j3d_file, h_shift, v_shift
                    )

                if hasattr(j3d_file, "mdl3"):
                    shift_all_colors_in_mdl3(
                        self, file_name, j3d_file, h_shift, v_shift
                    )

                if hasattr(j3d_file, "trk1"):
                    shift_all_colors_in_trk1(
                        self, file_name, j3d_file, h_shift, v_shift
                    )
...
def shift_all_colors_in_mdl3(self, file_name, j3d_file, h_shift, v_shift):
    for entry in j3d_file.mdl3.entries:
        tev_color_commands = [
            com
            for com in entry.bp_commands
            if com.register >= 0xE0 and com.register <= 0xE7
        ]
        assert (
            len(tev_color_commands) % 2 == 0
        )  # They should come in pairs of low and high
        ...
        for i in range(0, len(tev_color_commands), 2):
            ...
            if hi_command.register != lo_command.register + 1:
                ...
                assert (
                    last_hi_command.register
                    == hi_command.register
                    == lo_command.register
                )
                assert (
                    last_hi_command_orig_value == hi_command.value == lo_command.value
                )
...
def shift_all_colors_in_trk1(self, file_name, j3d_file, h_shift, v_shift):
    ...
    for anim_index, anim in enumerate(animations):
        ...
        assert (
            len(anim.r.keyframes) > 0
            and len(anim.g.keyframes) > 0
            and len(anim.b.keyframes) > 0
        )
        ...
        for keyframe_time in unique_keyframe_times:
            ...
            r = (
                get_keyframe_by_closest_time(anim.r.keyframes, keyframe_time).value
                & 0xFF
            )
            g = (
                get_keyframe_by_closest_time(anim.g.keyframes, keyframe_time).value
                & 0xFF
            )
            b = (
                get_keyframe_by_closest_time(anim.b.keyframes, keyframe_time).value
                & 0xFF
            )
            # print("    %d %d %d" % (r, g, b))
            r, g, b = texture_utils.hsv_shift_color((r, g, b), h_shift, v_shift)
...
def shift_hardcoded_darknut_colors(self, h_shift, v_shift):
    ...
    for palette_offset in [0x4540, 0x6560, 0x8580, 0xA5A0, 0xC5C0]:
        ...
        colors = texture_utils.decode_palettes(
            palette_data,
            texture_utils.PaletteFormat.RGB5A3,
            16,
