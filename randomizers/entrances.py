...
DUNGEON_EXITS = [
    ZoneExit(
        "M_NewD2", 0, 0, 0, "Dragon Roost Cavern", "Dragon Roost Cavern", "M_DragB"
    ),
...
SECRET_CAVE_EXITS = [
    ...
    ZoneExit(
        "TF_06", 0, 0, 0, "Dragon Roost Island", "Dragon Roost Island Secret Cave", None
    ),
    ...
    ZoneExit(
        "SubD42", 0, 0, 0, "Needle Rock Isle", "Needle Rock Isle Secret Cave", None
    ),
    ...
    ZoneExit(
        "TF_03", 0, 0, 0, "Bird's Peak Rock", "Bird's Peak Rock Secret Cave", None
    ),
    ...
    ZoneExit(
        "Cave05", 0, 0, 0, "Horseshoe Island", "Horseshoe Island Secret Cave", None
    ),
...
def randomize_entrances(self):
    if self.options.get("randomize_entrances") == "Dungeons":
        ...
        self.options.get("randomize_entrances")
        == "Dungeons & Secret Caves (Separately)"
    ):
        randomize_one_set_of_entrances(self, include_dungeons=True, include_caves=False)
        randomize_one_set_of_entrances(self, include_dungeons=False, include_caves=True)
    elif (
        self.options.get("randomize_entrances") == "Dungeons & Secret Caves (Together)"
    ):
...
def randomize_one_set_of_entrances(self, include_dungeons=False, include_caves=False):
    ...
    if self.dungeons_and_caves_only_start:
        ...
        if include_caves and (
            self.options.get("progression_puzzle_secret_caves")
    ...
    if self.options.get("race_mode"):
        ...
        for zone_entrance in relevant_entrances:
            for other_zone_entrance in relevant_entrances:
                if (
                    other_zone_entrance.island_name == zone_entrance.island_name
                    and other_zone_entrance != zone_entrance
                ):
    ...
    for zone_entrance in relevant_entrances:
        if (
            doing_progress_entrances_for_dungeons_and_caves_only_start
            and zone_entrance == safety_entrance
        ):
            possible_remaining_exits = [
                e
                for e in remaining_exits
                if e.unique_name in exit_names_with_no_requirements
            ]
        else:
        ...
        if self.options.get("race_mode"):
            ...
            done_entrances_on_same_island_leading_to_a_dungeon = [
                entr
                for entr in done_entrances_to_exits
            ...
            if done_entrances_on_same_island_leading_to_a_dungeon:
                possible_remaining_exits = [
                    x for x in possible_remaining_exits if x not in DUNGEON_EXITS
                ]
        ...
        self.dungeon_and_cave_island_locations[
            zone_exit.zone_name
        ] = zone_entrance.island_name
        done_entrances_to_exits[zone_entrance] = zone_exit

        if not self.dry_run:
            # Update the stage this entrance takes you into.
            entrance_dzx_path = "files/res/Stage/%s/Room%d.arc" % (
                zone_entrance.stage_name,
                zone_entrance.room_num,
            )
            entrance_dzx = self.get_arc(entrance_dzx_path).get_file("room.dzr")
            entrance_scls = entrance_dzx.entries_by_type("SCLS")[
                zone_entrance.scls_exit_index
            ]
            ...
            exit_dzx_path = "files/res/Stage/%s/Room%d.arc" % (
                zone_exit.stage_name,
                zone_exit.room_num,
            )
            ...
            if zone_exit.unique_name == "Savage Labyrinth":
                for stage_and_room_name in [
                    ...
                    exit_sclses = [
                        x
                        for x in savage_dzr.entries_by_type("SCLS")
                        if x.dest_stage_name == "sea"
                    ]
            ...
            if zone_exit.boss_stage_name is not None:
                ...
                boss_stage_arc_path = (
                    "files/res/Stage/%s/Stage.arc" % zone_exit.boss_stage_name
                )
                event_list = self.get_arc(boss_stage_arc_path).get_file(
                    "event_list.dat"
                )
                warp_out_event = event_list.events_by_name["WARP_WIND_AFTER"]
                director = next(
                    actor for actor in warp_out_event.actors if actor.name == "DIRECTOR"
                )
                stage_change_action = next(
                    action for action in director.actions if action.name == "NEXT"
                )
                stage_name_prop = next(
                    prop
