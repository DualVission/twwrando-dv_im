...
def randomize_enemies(self):
    ...
    self.enemies_to_randomize_to = [
        data for data in self.enemy_types if data["Allow randomizing to"]
    ]
    ...
    for stage_folder, enemy_locations in self.enemy_locations.items():
        ...
        while True:
            done_enemy_locations_for_stage = randomize_enemy_groups_for_stage(
                self, stage_folder, enemy_locations
            )
            ...
            if done_enemy_locations_for_stage != False:
                ...
                raise Exception(
                    "Enemy rando failed %d times for one stage (%s)"
                    % (stage_attempts, stage_folder)
                )
...
def decide_on_enemy_pool_for_stage(self, stage_folder, enemy_locations):
    ...
    for enemy_group in enemy_locations:
        if enemy_group["Must defeat enemies"]:
            ...
            enemies_logically_allowed_in_this_group = (
                self.logic.filter_out_enemies_that_add_new_requirements(
                    original_req_string,
        ...
        for enemy_location in enemy_group["Enemies"]:
            category_and_logic_combo = (
                enemy_location["Placement category"],
                enemies_logically_allowed_in_this_group,
            )
    ...
    for category, enemies_logically_allowed in category_and_logic_combos_needed:
        enemies_allowed_for_combo = [
            enemy_data
            for enemy_data in enemies_logically_allowed
    ...
    if num_species_chosen > MAX_ENEMY_SPECIES_PER_STAGE:
        raise Exception(
            "Enemy species pool for %s has %d species in it instead of %d"
            % (stage_folder, num_species_chosen, MAX_ENEMY_SPECIES_PER_STAGE)
        )
        ...
        for i in range(MAX_ENEMY_SPECIES_PER_STAGE - num_species_chosen):
            enemies_possible_for_this_stage_minus_chosen = [
                enemy_data
                for enemy_data in all_enemies_possible_for_this_stage
...
def randomize_enemy_groups_for_stage(self, stage_folder, enemy_locations):
    ...
    enemy_pool_for_stage = decide_on_enemy_pool_for_stage(
        self, stage_folder, enemy_locations
    )
    for enemy_group in enemy_locations:
        room_attempts = 0
        ...
        while True:
            done_enemy_locations_for_room = randomize_enemy_group(
                self, stage_folder, enemy_group, enemy_pool_for_stage
            )
...
def randomize_enemy_group(self, stage_folder, enemy_group, enemy_pool_for_stage):
    ...
    enemies_logically_allowed_in_this_group_not_yet_in_pool = (
        enemies_logically_allowed_in_this_group.copy()
    )
    all_enemies_possible_for_this_group = []
    for category in unique_categories_in_this_group:
        enemies_allowed = [
            enemy_data
            for enemy_data in enemies_logically_allowed_in_this_group_not_yet_in_pool
            if is_enemy_allowed_in_placement_category(enemy_data, category)
    ...
    if num_species_chosen > MAX_ENEMY_SPECIES_PER_GROUP:
        raise Exception(
            "Enemy species pool for group has %d species in it instead of %d"
            % (num_species_chosen, MAX_ENEMY_SPECIES_PER_GROUP)
        )
        ...
        for i in range(MAX_ENEMY_SPECIES_PER_GROUP - num_species_chosen):
            enemies_possible_for_this_group_minus_chosen = [
                enemy_data
                for enemy_data in all_enemies_possible_for_this_group
    ...
    for enemy_location in enemy_group["Enemies"]:
        enemy, arc_name, dzx, layer = get_enemy_instance_by_path(
            self, enemy_location["Path"]
        )
        _, room_arc_name = arc_name.split("/")

        enemies_to_randomize_to_for_this_location = [
            data
            for data in enemy_pool_for_group
            if is_enemy_allowed_in_placement_category(
                data, enemy_location["Placement category"]
            )
        ]
        ...
        if len(enemies_to_randomize_to_for_this_location) == 0:
            error_msg = (
                "No possible enemies to place in %s of the correct category\n"
                % arc_name
            )
            enemy_pretty_names_in_this_stage_pool = [
                enemy_data["Pretty name"] for enemy_data in enemy_pool_for_stage
            ]
            ...
            for data in enemies_logically_allowed_in_this_group:
                if (
                    data["Actor name"]
                    not in enemy_actor_names_logically_allowed_in_this_group
                ):
                    enemy_actor_names_logically_allowed_in_this_group.append(
                        data["Actor name"]
                    )
            ...
            enemy_pretty_names_in_this_group_pool = [
                enemy_data["Pretty name"] for enemy_data in enemy_pool_for_group
            ]
            ...
            error_msg += "Enemies of the correct category ({}): {}".format(
                enemy_location["Placement category"],
                ", ".join(enemy_actor_names_of_correct_category),
            )
        ...
        if DEBUG_ENEMY_NAME_TO_PLACE_EVERYWHERE is not None:
            new_enemy_data = self.enemy_datas_by_pretty_name[
                DEBUG_ENEMY_NAME_TO_PLACE_EVERYWHERE
            ]

        # Account for the amount of memory used up by this enemy instance.
        free_memory -= get_amount_of_memory_for_enemy(
            new_enemy_data, enemy_actor_names_already_placed_in_room
        )
        if False:
            ...
            print(
                "Subtracted: %d"
                % get_amount_of_memory_for_enemy(
                    new_enemy_data, enemy_actor_names_already_placed_in_room
                )
            )

        if (
            free_memory < MIN_FREE_SPACE_TO_LEAVE_PER_ROOM
            and DEBUG_ENEMY_NAME_TO_PLACE_EVERYWHERE is None
        ):
        ...
        if new_enemy_data["Actor name"] not in enemy_actor_names_already_placed_in_room:
            ...
            enemy_actor_names_already_placed_in_room.append(
                new_enemy_data["Actor name"]
            )
...
def save_changed_enemies_and_randomize_their_params(self):
    ...
    for enemy_location, new_enemy_data in self.done_enemy_locations:
        ...
        if False:
            ...
            print(
                "Putting a {} (param:{:08X}) in {}".format(
                    new_enemy_data["Actor name"], new_enemy_data["Params"], path
                )
            )
        ...
        if ":" in placement_category:
            ...
            if "SetsDeathSwitch" in conditions:
                ...
                if death_switch_param_name is None:
                    raise Exception(
                        "An enemy location specified that it must set a switch on death, but the original enemy there is not documented to be able to set a switch on death."
                    )

                death_switch_to_set = getattr(enemy, death_switch_param_name)
                if (
                    death_switch_to_set in [0x00, 0x80, 0xFF]
                    or death_switch_to_set >= 0xF0
                ):
                    raise Exception(
                        "Switch index to set on enemy death is not valid for all enemy types: %02X"
                        % death_switch_to_set
                    )
        ...
        randomize_enemy_params(
            self, new_enemy_data, enemy, placement_category, dzx, layer
        )
        ...
        if death_switch_to_set is not None:
            ...
            if death_switch_param_name is None:
                if DEBUG_ENEMY_NAME_TO_PLACE_EVERYWHERE is None:
                    raise Exception(
                        "Tried to place an enemy type that cannot set a switch on death in a location that requires a switch be set on death: %s"
                        % enemy.name
                    )
            else:
        ...
        if stage_folder == "sea":
            dzr = self.get_arc("files/res/Stage/sea/" + room_arc_name).get_file(
                "room.dzr"
            )
            dest_jpc_index = dzr.entries_by_type("FILI")[0].loaded_particle_bank
        else:
            dzs = self.get_arc(
                "files/res/Stage/" + stage_folder + "/Stage.arc"
            ).get_file("stage.dzs")
        ...
        for particle_id in new_enemy_data["Required particle IDs"]:
            if (
                particle_id
                not in self.particles_to_load_for_each_jpc_index[dest_jpc_index]
            ):
                self.particles_to_load_for_each_jpc_index[dest_jpc_index].append(
                    particle_id
                )
        ...
        for enemy_group in enemy_locations:
            ...
            for actor_path in actor_paths:
                ...
                if actor.name == "AND_SW0":
                    ...
                    raise Exception(
                        "Unimplemented switch-setting actor name: %s" % actor.name
                    )
    ...
    for stage_folder, enemy_locations in self.enemy_locations.items():
        for enemy_group in enemy_locations:
            ...
            for new_actor_data in new_actor_datas:
                dzx, _, layer = get_dzx_fourcc_and_layer_by_path(
                    self, enemy_group["Enemies"][0]["Path"]
                )
    ...
    for dest_jpc_index, particle_ids in particles_to_load_for_each_jpc_index.items():
        for particle_id in particle_ids:
            ...
            if particle_id in particle_and_textures_by_id:
                ...
                if particle is None:
                    raise Exception(
                        "Failed to find a particle with ID %04X in any of the game's JPC files."
                        % particle_id
                    )
...
def print_all_enemy_params(self):
    ...
    for dzx, arc_path in stage_searcher.each_stage_and_room(self):
        ...
        for enemy in enemies:
            print(
                f"{enemy.name: 7}  {enemy.params:08X}  {enemy.aux_params_1:04X}  {enemy.aux_params_2:04X}  {arc_path}"
            )
    ...
    for dzx, arc_path in stage_searcher.each_stage_and_room(self):
        for layer in [None] + list(range(11 + 1)):
            ...
            enemies = [
                ...
                and get_enemy_data_for_actor(self, actor)[
                    "Allow randomizing from"
                ]  # Don't list unrandomizable enemies
            ]
            ...
            if stage_folder != prev_stage_folder or (
                stage_folder == "sea" and arc_name != prev_arc_name
            ):
            ...
            for enemy in enemies:
                defeat_reqs = get_enemy_data_for_actor(self, enemy)[
            ...
            and_sws = [
                actor for actor in actors if actor.name in ["AND_SW0", "AND_SW2"]
            ]
            if and_sws:
                ...
                for and_sw in and_sws:
                    ...
                    and_sw_path = (
                        relative_arc_path + layer_name + "/Actor%03X" % actor_index
                    )
            ...
            for enemy in enemies:
                ...
                placement_category = get_placement_category_for_vanilla_enemy_location(
...
def get_enemy_data_for_actor(self, enemy):
    ...
    raise Exception(
        f'Unknown enemy subspecies: actor name "{enemy.name}", params {enemy.params:08X}, aux params {enemy.aux_params_1:04X}, aux params 2 {enemy.aux_params_2:04X}'
    )
...
def get_placement_category_for_vanilla_enemy_location(self, enemy_data, enemy):
    ...
    raise Exception(
        f'Unknown placement category for enemy: actor name "{enemy.name}", params {enemy.params:08X}, aux params {enemy.aux_params_1:04X}, aux params 2 {enemy.aux_params_2:04X}'
    )
...
def get_amount_of_memory_for_enemy(
    enemy_data, enemy_actor_names_already_placed_in_room
):
...
def randomize_enemy_params(self, enemy_data, enemy, category, dzx, layer):
    if enemy.name == "Bk":
        ...
        enemy.poe_floats = self.rng.choice(
            [0, 1]
        )  # 0 here will make it hover down if it's placed in the air (though the rando won't place them in the air so it shouldn't change anything).
        ...
        if enemy_data["Pretty name"] == "Winged Mothula":
            ...
            wing_indexes_to_be_missing = wing_indexes_to_be_missing[
                0:number_of_wings_to_be_missing
            ]
...
def adjust_enemy(self, enemy_data, enemy, category, dzx, layer):
    ...
    if enemy.name == "magtail":
        ...
        enemy.name in ["c_green", "c_red", "c_kiiro", "c_blue", "c_black"]
        and category == "Pot"
    ):
        ...
        pots_on_same_layer = [
            actor
            for actor in dzx.entries_by_type_and_layer("ACTR", layer)
            if actor.actor_class_name == "d_a_tsubo"
        ]
        if not pots_on_same_layer:
            raise Exception("No pots on same layer as ChuChu in a pot")
        closest_pot = min(
            pots_on_same_layer, key=lambda pot: distance_between_entities(enemy, pot)
        )
...
def get_enemy_instance_by_path(self, path):
    match = re.search(
        r"^([^/]+/[^/]+\.arc)(?:/Layer([0-9a-b]))?/Actor([0-9A-F]{3})$", path
    )
    if not match:
    ...
    if enemy.name not in self.all_enemy_actor_names:
        raise Exception(
            f"Enemy location path {path} points to a {enemy.name} actor, not an enemy!"
        )
...
def get_actor_by_path(self, path):
    match = re.search(
        r"^([^/]+/[^/]+\.arc)(?:/Layer([0-9a-b]))?/(Actor|ScalableObject|Door|ScalableDoor)([0-9A-F]{3})$",
        path,
    )
    if not match:
        raise Exception("Invalid actor path: %s" % path)
...
class EnemyCategory:
    ...
    def __eq__(self, other):
        ...
        for other_condition in other_conditions:
            if other_condition == "SetsDeathSwitch":
                ...
                raise NotImplementedError(
                    "Enemy placement category condition type not implemented: %s"
                    % other_condition
                )
    ...
    def __str__(self):
        return (
            f"EnemyCategory({self.category_string}, DeathSwitch: {self.can_set_switch})"
        )
