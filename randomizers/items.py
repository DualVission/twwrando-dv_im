...
def randomize_items(self):
    ...
    while self.logic.unplaced_nonprogress_items:
        try:
            ...
            possible_locations = self.logic.filter_locations_valid_for_item(
    ...
    inaccessible_locations = [
        loc
        for loc in self.logic.remaining_item_locations
    ...
    for location_name in locations_to_place_consumables_at:
        possible_items = self.logic.filter_items_valid_for_location(
            self.logic.unplaced_fixed_consumable_items, location_name
        )
        if len(possible_items) == 0:
            possible_items = self.logic.filter_items_valid_for_location(
                self.logic.duplicatable_consumable_items, location_name
            )
            if len(possible_items) == 0:
                raise Exception(
...
def randomize_boss_rewards(self):
    ...
    triforce_shards = [
        item_name
        for item_name in unplaced_progress_items_degrouped
    ...
    if len(boss_reward_items) != total_num_rewards:
        raise Exception(
            "Number of boss reward items is incorrect: " + ", ".join(boss_reward_items)
        )
    ...
    if len(possible_boss_locations) != 6:
        raise Exception(
            "Number of boss item locations is incorrect: "
    ...
    for item_name in boss_reward_items:
        ...
        if item_name == "Hookshot":
            possible_boss_locations_for_this_item = [
                loc
                for loc in possible_boss_locations_for_this_item
        ...
        if (
            self.dungeons_only_start
            and "Dragon Roost Cavern - Gohma Heart Container"
            in possible_boss_locations_for_this_item
        ):
            location_name = "Dragon Roost Cavern - Gohma Heart Container"
        elif (
            self.dungeons_only_start
            and "Forbidden Woods - Kalle Demos Heart Container"
            in possible_boss_locations_for_this_item
        ):
    ...
    for location_name in self.logic.item_locations:
        ...
        if (
            self.logic.is_dungeon_location(location_name)
            and zone_name in banned_dungeons
        ):
            self.race_mode_banned_locations.append(location_name)
        elif (
            location_name == "Mailbox - Letter from Orca"
            and "Forbidden Woods" in banned_dungeons
        ):
            self.race_mode_banned_locations.append(location_name)
        elif (
            location_name == "Mailbox - Letter from Baito"
            and "Earth Temple" in banned_dungeons
        ):
            self.race_mode_banned_locations.append(location_name)
        elif (
            location_name == "Mailbox - Letter from Aryll"
            and "Forsaken Fortress" in banned_dungeons
        ):
            self.race_mode_banned_locations.append(location_name)
        elif (
            location_name == "Mailbox - Letter from Tingle"
            and "Forsaken Fortress" in banned_dungeons
        ):
            self.race_mode_banned_locations.append(location_name)
...
def randomize_dungeon_items(self):
    ...
    items_to_temporarily_add = [
        item_name
        for item_name in (
            self.logic.unplaced_progress_items + self.logic.unplaced_nonprogress_items
        )
    ...
    if self.options.get("compass_map_pool_with_keys"):
        other_dungeon_items_to_place = [
            item_name
            for item_name in (
                self.logic.unplaced_progress_items
                + self.logic.unplaced_nonprogress_items
            )
            if item_name.endswith(" Compass") or item_name.endswith(" Dungeon Map")
        ]
...
def place_dungeon_item(self, item_name):
    ...
    if self.dungeons_only_start and item_name in [
        "DRC Big Key",
        "DRC Dungeon Map",
        "DRC Compass",
    ]:
        ...
        possible_locations = [
            loc for loc in possible_locations if loc != self.drc_failsafe_location
        ]
...
def randomize_progression_items(self):
    ...
    while self.logic.unplaced_progress_items:
        ...
        if newly_accessible_predetermined_item_locations:
            for (
                ...
                predetermined_item_name = self.logic.prerandomization_item_locations[
                    predetermined_item_location_name
                ]
                self.logic.set_location_to_item(
        ...
        unfound_prerand_locs = [
            loc
            for loc in self.logic.prerandomization_item_locations
        ...
        possible_items = self.logic.filter_items_by_any_valid_location(
            possible_items, accessible_undone_locations
        )

        if len(possible_items) == 0:
            raise Exception(
                "No valid locations left for any of the unplaced progress items!"
            )
        ...
        possible_groups = [
            name for name in possible_items if name in self.logic.progress_item_groups
        ]
        useless_groups = self.logic.get_all_useless_items(possible_groups)
        possible_items_when_not_placing_useful = [
            name for name in possible_items if name not in useless_groups
        ]
            ...
            if item_name is None:
                if must_place_useful_item:
                    ...
                    item_by_usefulness_fraction = (
                        self.logic.get_items_by_usefulness_fraction(
                            possible_items_when_not_placing_useful
                        )
                    )
                    ...
                    items_at_max_usefulness = [
                        item_name
                        for item_name, usefulness in item_by_usefulness_fraction.items()
        ...
        if self.options.get("race_mode"):
            ...
            if len(locations_filtered) >= num_locs_needed:
                ...
                raise Exception(
                    "Failed to prevent progress items from appearing in unchosen dungeons for race mode."
                )
        ...
        if item_name in self.logic.progress_item_groups:
            ...
            self.logic.set_multiple_locations_to_group(
                possible_locations_for_group, group_name
            )
        else:
            possible_locations = self.logic.filter_locations_valid_for_item(
                accessible_undone_locations, item_name
            ...
            if item_name.startswith("Treasure Chart") or item_name.startswith(
                "Triforce Chart"
            ):
    ...
    for location_name in self.logic.prerandomization_item_locations:
        if location_name in self.logic.remaining_item_locations:
            dungeon_item_name = self.logic.prerandomization_item_locations[
                location_name
            ]
...
def change_item(self, path, item_name):
    ...
    chest_match = re.search(
        r"^([^/]+/[^/]+\.arc)(?:/Layer([0-9a-b]))?/Chest([0-9A-F]{3})$", path
    )
    event_match = re.search(
        r"^([^/]+/[^/]+\.arc)/Event([0-9A-F]{3}):[^/]+/Actor([0-9A-F]{3})/Action([0-9A-F]{3})$",
        path,
    )
    scob_match = re.search(
        r"^([^/]+/[^/]+\.arc)(?:/Layer([0-9a-b]))?/ScalableObject([0-9A-F]{3})$", path
    )
    ...
    if rel_match:
        ...
        change_event_item(
            self, arc_path, event_index, actor_index, action_index, item_id
        )
    elif scob_match:
