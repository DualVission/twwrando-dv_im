...
def randomize_charts(self):
    ...
    randomizable_charts = [
        chart for chart in self.chart_list.charts if chart.type in [0, 1, 2, 6]
    ]
    ...
    for chart in randomizable_charts:
        ...
        for random_pos_index in range(4):
            ...
            possible_pos_to_copy_from = chart_to_copy_from.possible_random_positions[
                random_pos_index
            ]

            possible_pos.chart_texture_x_offset = (
                possible_pos_to_copy_from.chart_texture_x_offset
            )
            possible_pos.chart_texture_y_offset = (
                possible_pos_to_copy_from.chart_texture_y_offset
            )
        ...
        dzx = self.get_arc(
            "files/res/Stage/sea/Room%d.arc" % chart.island_number
        ).get_file("room.dzr")
        ...
        self.island_number_to_chart_name[
            chart_to_copy_from.island_number
        ] = chart.item_name
