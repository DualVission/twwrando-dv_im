...
class J3DChunk:
    ...
    def read_string_table(self, string_table_offset):
        ...
        for i in range(num_strings):
            ...
            string = read_str_until_null_character(
                self.data, string_table_offset + string_data_offset
            )
    ...
    def write_string_table(self, string_table_offset, strings):
        ...
        for string in strings:
            ...
            write_str_with_null_byte(
                self.data, string_table_offset + next_string_data_offset, string
            )
...
class TEX1(J3DChunk):
    ...
    def save_chunk_specific_data(self):
        ...
        next_available_data_offset = (
            self.texture_header_list_offset + self.num_textures * 0x20
        )  # Right after the last header ends
        ...
        for i, texture in enumerate(self.textures):
            ...
            if filename in image_data_offsets:
                texture.image_data_offset = (
                    image_data_offsets[filename] - texture.header_offset
                )
                continue
            ...
            texture.image_data_offset = (
                next_available_data_offset - texture.header_offset
            )
        ...
        for i, texture in enumerate(self.textures):
            ...
            if filename in palette_data_offsets:
                texture.palette_data_offset = (
                    palette_data_offsets[filename] - texture.header_offset
                )
                continue
            ...
            if texture.needs_palettes():
                ...
                texture.palette_data_offset = (
                    first_texture.image_data_offset
                    + first_texture.header_offset
                    - texture.header_offset
                )
                palette_data_offsets[filename] = (
                    first_texture.image_data_offset + first_texture.header_offset
                )
...
class MAT3(J3DChunk):
    def read_chunk_specific_data(self):
        ...
        self.num_reg_colors = (
            self.tev_konst_colors_offset - self.tev_reg_colors_offset
        ) // 8
        self.reg_colors = []
        ...
        self.num_konst_colors = (
            self.tev_stages_offset - self.tev_konst_colors_offset
        ) // 4
        self.konst_colors = []
...
class ColorAnimation:
    ...
    def read(
        self, data, offset, r_track_data, g_track_data, b_track_data, a_track_data
    ):
