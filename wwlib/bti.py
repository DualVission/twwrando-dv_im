...
class BTI:
    def __init__(self, data, header_offset=0):
        ...
        self.image_data = BytesIO(
            read_bytes(data, header_offset + self.image_data_offset, image_data_size)
        )

        palette_data_size = self.num_colors * 2
        self.palette_data = BytesIO(
            read_bytes(
                data, header_offset + self.palette_data_offset, palette_data_size
            )
        )
    ...
    def read_header(self, data, header_offset=0):
        ...
        self.max_lod = read_u8(
            data, header_offset + 0x17
        )  # seems to be equal to (mipmap_count-1)*8
    ...
    def render_palette(self):
        colors = decode_palettes(
            self.palette_data, self.palette_format, self.num_colors, self.image_format
        )
        return colors
    ...
    def replace_palette(self, new_colors):
        encoded_colors = generate_new_palettes_from_colors(
            new_colors, self.palette_format
        )
        self.palette_data = encode_palette(
            encoded_colors, self.palette_format, self.image_format
        )
        self.num_colors = len(encoded_colors)
...
class BTIFile(
    BTI
):  # For standalone .bti files (as opposed to textures embedded inside J3D models/animations)
