...
class RARC:
    ...
    def read(self, data):
        ...
        self.node_list_offset = (
            read_u32(data, self.data_header_offset + 0x04) + self.data_header_offset
        )
        self.total_num_file_entries = read_u32(data, self.data_header_offset + 0x08)
        self.file_entries_list_offset = (
            read_u32(data, self.data_header_offset + 0x0C) + self.data_header_offset
        )
        self.string_list_size = read_u32(data, self.data_header_offset + 0x10)
        self.string_list_offset = (
            read_u32(data, self.data_header_offset + 0x14) + self.data_header_offset
        )
        ...
        for file_index in range(self.total_num_file_entries):
            file_entry_offset = (
                self.file_entries_list_offset + file_index * FileEntry.ENTRY_SIZE
            )
        ...
        for node in self.nodes:
            for file_index in range(
                node.first_file_index, node.first_file_index + node.num_files
            ):
    ...
    def add_new_directory(self, dir_name, node_type, parent_node):
        if len(node_type) > 4:
            raise Exception(
                "Node type must not be longer than 4 characters: %s" % node_type
            )
        if len(node_type) < 4:
        ...
        for file in node.files:
            if file.is_dir:
                if file.name not in [".", ".."]:
                    subdir_path = os.path.join(path, file.name)
                    subdir_node = self.nodes[file.node_index]
                    num_files_overwritten += self.import_node_from_disk(
                        subdir_node, subdir_path
                    )
            else:
                file_path = os.path.join(path, file.name)
    ...
    def save_changes(self):
        ...
        for file_entry in self.nodes + self.file_entries:
            ...
            if string in offsets_for_already_written_strings:
                ...
                write_str_with_null_byte(
                    self.data, self.string_list_offset + offset, string
                )
        ...
        for file_entry in self.file_entries:
            if file_entry.is_dir:
                ...
                if file_entry.type & RARCFileAttrType.PRELOAD_TO_MRAM != 0:
                    ...
                    raise Exception(
                        "File entry %s is not set as being loaded into any type of RAM."
                        % file_entry.name
                    )
        ...
        write_u32(
            self.data,
            self.data_header_offset + 0x04,
            self.node_list_offset - self.data_header_offset,
        )
        write_u32(
            self.data, self.data_header_offset + 0x08, self.total_num_file_entries
        )
        write_u32(
            self.data,
            self.data_header_offset + 0x0C,
            self.file_entries_list_offset - self.data_header_offset,
        )
        ...
        write_u32(
            self.data,
            self.data_header_offset + 0x14,
            self.string_list_offset - self.data_header_offset,
        )
        write_u16(self.data, self.data_header_offset + 0x18, self.next_free_file_id)
        write_u8(
            self.data,
            self.data_header_offset + 0x1A,
            self.keep_file_ids_synced_with_indexes,
        )
...
class Node:
    ...
    def read(self, node_offset):
        ...
        self.name = read_str_until_null_character(
            self.rarc.data, self.rarc.string_list_offset + self.name_offset
        )
...
class FileEntry:
    ...
    def save_changes(self):
        ...
        write_u32(
            self.rarc.data, self.entry_offset + 0x10, 0
        )  # Pointer to the file's data, filled at runtime.
