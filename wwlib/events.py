...
class EventList:
    ...
    def __init__(self, file_entry):
        ...
        for event in self.events:
            ...
            for actor_index in event.actor_indexes:
                if actor_index == -1:
                    ...
                    if found_blank:
                        raise Exception(
                            "Found a non-blank actor after a blank actor in an event's actor indexes list"
                        )
        ...
        for property in all_properties:
            if property.data_type == 0:
                ...
                raise Exception(
                    "Reading property data type %d not implemented" % property.data_type
                )
        ...
        for i, property in enumerate(all_properties):
            ...
            if isinstance(property_value, str):
                ...
                if first_val_class == float:
                    ...
                    raise Exception(
                        "Unknown type of property {}: {}".format(
                            property.name, repr(property_value)
                        )
                    )
            else:
                raise Exception(
        ...
        for property in all_properties:
            if property.data_type == 4:
                ...
                if string_length_with_null % 8 != 0:
                    ...
                    string_length_with_padding = (
                        string_length_with_null + padding_bytes_needed
                    )
                else:
    ...
    def get_unused_flag_id(self):
        if not self.unused_flag_ids:
            raise Exception(
                "No unused flags left for adding new actor/action to event_list.dat!"
            )
...
class Event:
    ...
    def read(self, offset):
        ...
        self.actors = (
            []
        )  # This will be populated by the event list after it reads the actors.
