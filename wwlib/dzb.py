    ...
    def read(self, data):
        ...
        for block_index, block in enumerate(self.octree_blocks):
            ...
            for face_index in range(
                block.first_face_index, next_block_first_face_index
            ):
    ...
    def generate_octree_node(self, faces):
        ...
        for i in range(8):
            ...
            for face in remaining_faces:
                face_center = (
                    (
                        face.vertices[0].x_pos
                        + face.vertices[1].x_pos
                        + face.vertices[2].x_pos
                    )
                    / 3,
                    (
                        face.vertices[0].y_pos
                        + face.vertices[1].y_pos
                        + face.vertices[2].y_pos
                    )
                    / 3,
                    (
                        face.vertices[0].z_pos
                        + face.vertices[1].z_pos
                        + face.vertices[2].z_pos
                    )
                    / 3,
                )
                if (
                    face_center[0] < octant_min[0]
                    or face_center[1] < octant_min[1]
                    or face_center[2] < octant_min[2]
                ):
                    continue
                if (
                    face_center[0] > octant_max[0]
                    or face_center[1] > octant_max[1]
                    or face_center[2] > octant_max[2]
                ):
                    continue
...
class Group:
    ...
    def read(self, offset):
        ...
        self.unknown_1 = read_u16(
            self.dzb_data, self.offset + 0x16
        )  # Usually 0xFFFF, but sometimes 0xDCDC.
...
class Property:
    ...
    def read(self, offset):
        ...
        self.pass_0_normal = (
            bitfield_4 & 0x00000002
        ) != 0  # Normal, covers most things
        self.pass_2_link = (bitfield_4 & 0x00000004) != 0  # Link
        self.pass_3_arrows_and_light = (
            bitfield_4 & 0x00000008
        ) != 0  # Link's arrows and reflected lightrays. Also prevents actor shadows from being drawn on this surface.
        self.hookshottable = (
            bitfield_4 & 0x00000010
        ) != 0  # Allows the hookshot to stick to this surface
        self.pass_4_bombs = (bitfield_4 & 0x00000020) != 0  # Bombs
        self.pass_5_boomerang = (bitfield_4 & 0x00000040) != 0  # Boomerang
        self.pass_6_hookshot = (
            bitfield_4 & 0x00000080
        ) != 0  # Hookshot. Also might cover Link's line of sight...? (mRopeLinChk in daPy_lk_c)
