...
class GCM:
    ...
    def read_filesystem(self):
        ...
        for file_index in range(num_file_entries):
            ...
            file_entry.read(
                file_index, self.iso_file, file_entry_offset, self.fnt_offset
            )
    ...
    def read_system_data(self):
        ...
        apploader_full_size = (
            apploader_header_size + apploader_size + apploader_trailer_size
        )
        self.files_by_path["sys/apploader.img"] = SystemFile(
            0x2440, apploader_full_size, "apploader.img"
        )
        ...
        for i in range(11):  # Data sections
            ...
            if section_end_offset > main_dol_size:
                main_dol_size = section_end_offset
        self.files_by_path["sys/main.dol"] = SystemFile(
            dol_offset, main_dol_size, "main.dol"
        )

        self.files_by_path["sys/fst.bin"] = SystemFile(
            self.fst_offset, self.fst_size, "fst.bin"
        )
    ...
    def read_file_data(self, file_path):
        ...
        with open(self.iso_path, "rb") as iso_file:
            data = read_bytes(
                iso_file, file_entry.file_data_offset, file_entry.file_size
            )
        data = BytesIO(data)
    ...
    def export_disc_to_folder_with_changed_files(
        self, output_folder_path, only_changed_files=False
    ):
        files_done = 0
            ...
            if file_path in self.changed_files:
                ...
                with open(full_file_path, "wb") as f:
                    while size_remaining > 0:
                        size_to_read = min(
                            size_remaining, MAX_DATA_SIZE_TO_READ_AT_ONCE
                        )

                        with open(self.iso_path, "rb") as iso_file:
                            data = read_bytes(
                                iso_file,
                                file_entry.file_data_offset + offset_in_file,
                                size_to_read,
                            )
                        f.write(data)
    ...
    def export_disc_to_iso_with_changed_files(self, output_file_path):
        if os.path.realpath(self.iso_path) == os.path.realpath(output_file_path):
            raise Exception(
                "Input ISO path and output ISO path are the same. Aborting."
            )
    ...
    def export_system_data_to_iso(self):
        ...
        for file_index, file_entry in enumerate(self.file_entries):
            ...
            if file_entry.is_dir:
                write_u32(
                    self.output_iso, file_entry_offset + 4, file_entry.parent_fst_index
                )
                write_u32(
                    self.output_iso, file_entry_offset + 8, file_entry.next_fst_index
                )
            ...
            if file_index != 0:  # Root doesn't have a name
                write_str_with_null_byte(
                    self.output_iso, next_name_offset, file_entry.name
                )
        ...
        write_u32(
            self.output_iso, 0x42C, self.fst_size
        )  # Seems to be a duplicate size field that must also be updated
    ...
    def export_filesystem_to_iso(self):
        ...
        file_entries_by_data_order = [
            file_entry for file_entry in self.file_entries if not file_entry.is_dir
        ]
    ...
    def read(self, file_index, iso_file, file_entry_offset, fnt_offset):
        ...
        if file_index == 0:
            ...
            self.name = read_str_until_null_character(
                iso_file, fnt_offset + self.name_offset
            )
