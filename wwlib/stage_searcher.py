...
def each_stage_and_room(
    self,
    exclude_stages=False,
    exclude_rooms=False,
    stage_name_to_limit_to=None,
    exclude_unused=True,
):
    ...
    all_filenames.sort(
        key=lambda filename: [
            try_int_convert(c) for c in re.split("([0-9]+)", filename)
        ]
    )
    ...
    for filename in all_filenames:
        stage_match = re.search(
            r"files/res/Stage/([^/]+)/Stage.arc", filename, re.IGNORECASE
        )
        room_match = re.search(
            r"files/res/Stage/([^/]+)/Room\d+.arc", filename, re.IGNORECASE
        )
        ...
        if stage_match:
            ...
            if self.stage_names[stage_name] == "Broken" or (
                exclude_unused and self.stage_names[stage_name] == "Unused"
            ):
...
def each_stage_with_rooms(self, exclude_unused=True):
    for dzs, stage_arc_path in each_stage(self, exclude_unused=exclude_unused):
        match = re.search(
            r"files/res/Stage/([^/]+)/Stage.arc", stage_arc_path, re.IGNORECASE
        )
        stage_name = match.group(1)

        rooms = []
        for dzr, room_arc_path in each_stage_and_room(
            self,
            exclude_stages=True,
            stage_name_to_limit_to=stage_name,
        ...
        for dzx, arc_path in [(dzs, stage_arc_path)] + rooms:
            for layer in [None] + list(range(11 + 1)):
                ...
                for actor in actors:
                    ...
                    location_identifier += (
                        "  in " + arc_path[len("files/res/Stage/") : -len(".arc")]
                    )
                    if layer is not None:
                        location_identifier += "/Layer%X" % layer

                    if is_unused:
                        used_item_flags_by_stage_id_unused[stage_id].append(
                            (item_flag, item_name, location_identifier)
                        )
                    else:
                        used_item_flags_by_stage_id[stage_id].append(
                            (item_flag, item_name, location_identifier)
                        )

    def write_used_item_flags_to_file(used_item_flags_dict, filename):
        used_item_flags_dict = OrderedDict(
            sorted(used_item_flags_dict.items(), key=lambda x: x[0])
        )
        ...
        with open(filename, "w") as f:
            ...
            for stage_id, item_flags in used_item_flags_dict.items():
                ...
                for item_flag, item_name, location_identifier in item_flags:
                    f.write(
                        f"  {item_flag:02X} for  {item_name: <21} {location_identifier}\n"
                    )

    write_used_item_flags_to_file(
        used_item_flags_by_stage_id, "Used item pickup flags by stage ID.txt"
    )
    write_used_item_flags_to_file(
        used_item_flags_by_stage_id_unused,
        "Used item pickup flags by stage ID (unused stages).txt",
    )
    ...
    for dzs, stage_arc_path, rooms in each_stage_with_rooms(self):
        ...
        for dzx, arc_path in [(dzs, stage_arc_path)] + rooms:
            ...
            for chest in chests:
                ...
                used_chest_flags_by_stage_id[stage_id_for_chest].append(
                    (chest.opened_flag, item_name, arc_path)
                )

    used_chest_flags_by_stage_id = OrderedDict(
        sorted(used_chest_flags_by_stage_id.items(), key=lambda x: x[0])
    )
    print()
            ...
            for actor in event.actors:
                ...
                for action in actor.actions:
                    ...
                    for prop in action.properties:
                        ...
                        if (
                            prop_value_str
                            not in all_actors[actor.name][action.name][prop.name]
                        ):
...
def print_stages_for_each_stage_id(self):
    ...
    stage_names_by_stage_id = OrderedDict(
        sorted(stage_names_by_stage_id.items(), key=lambda x: x[0])
    )
    print()
...
def print_actor_info(self):
    ...
    while True:
        ...
        rel_filename = self.dol.read_data(
            read_str_until_null_character, rel_filename_pointer
        )
    ...
    with open("Actor Info.txt", "w") as f:
        ...
        for i in range(0x339):
            ...
            f.write(
                "{:>7}:   ID {:04X},   Subtype {:02X},   GBAName {:02X},   REL {}\n".format(
                    actr_name, actor_id, subtype_index, gba_name, rel_filename
                )
            )
        ...
        for actor_id, rel_filename in actor_id_to_rel_filename.items():
            if actor_id not in done_actor_ids:
                ...
                f.write(
                    " [none]:   ID {:04X},   Subtype [],   GBAName [],   REL {}\n".format(
                        actor_id, rel_filename
                    )
                )
...
def print_all_entity_params(self):
    with open("All Entity Params.txt", "w") as f:
        ...
        for dzs, stage_arc_path, rooms in each_stage_with_rooms(
                ...
                for chunk_type in [
                    ...
                    for layer in [None] + list(range(11 + 1)):
                        for i, entity in enumerate(
                            dzx.entries_by_type_and_layer(chunk_type, layer)
                        ):
                            arc_path_short = arc_path[
                                len("files/res/Stage/") : -len(".arc")
                            ]
