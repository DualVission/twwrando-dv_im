...
def create_limited_palette_from_image(image, max_colors, with_alpha=True):
    ...
    if max_colors == 16:
        ...
        raise Exception(
            "Unsupported maximum number of colors to generate a palette for: %d"
            % max_colors
        )
...
def split_colors_into_buckets(all_pixel_colors, depth):
    ...
    r_range = max(r for r, g, b, a in all_pixel_colors) - min(
        r for r, g, b, a in all_pixel_colors
    )
    g_range = max(g for r, g, b, a in all_pixel_colors) - min(
        g for r, g, b, a in all_pixel_colors
    )
    b_range = max(b for r, g, b, a in all_pixel_colors) - min(
        b for r, g, b, a in all_pixel_colors
    )
...
def generate_new_palettes_from_image(image, image_format, palette_format):
    ...
    if len(encoded_colors) > MAX_COLORS_FOR_IMAGE_FORMAT[image_format]:
        ...
        limited_palette = create_limited_palette_from_image(
            image, MAX_COLORS_FOR_IMAGE_FORMAT[image_format], with_alpha=with_alpha
        )
...
def encode_palette(encoded_colors, palette_format, image_format):
    ...
    if len(encoded_colors) > MAX_COLORS_FOR_IMAGE_FORMAT[image_format]:
        raise TooManyColorsError(
            ...
            % (
                image_format.name,
                MAX_COLORS_FOR_IMAGE_FORMAT[image_format],
                len(encoded_colors),
            )
        )
...
def decode_image(
    ...
    while block_y < image_height:
        pixel_color_data = decode_block(
            image_format, image_data, offset, block_data_size, colors
        )
...
def decode_cmpr_block(image_format, image_data, offset, block_data_size, colors):
    ...
    for subblock_index in range(4):
        ...
        for i in range(16):
            ...
            x_in_subblock = i % 4
            y_in_subblock = i // 4
            pixel_index_in_block = (
                subblock_x + subblock_y * 8 + y_in_subblock * 8 + x_in_subblock
            )
...
def encode_image_from_path(
    new_image_file_path, image_format, palette_format, mipmap_count=1
):
    image = Image.open(new_image_file_path)
    image_width, image_height = image.size
    new_image_data, new_palette_data, encoded_colors = encode_image(
        image, image_format, palette_format, mipmap_count=mipmap_count
    )
...
def encode_image(image, image_format, palette_format, mipmap_count=1):
    ...
    encoded_colors, colors_to_color_indexes = generate_new_palettes_from_image(
        image, image_format, palette_format
    )
...
def encode_image_to_block(
    ...
    if image_format == ImageFormat.I4:
        return encode_image_to_i4_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_i8_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_ia4_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_ia8_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_rgb563_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_rgb5a3_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_rgba32_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_c4_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_c8_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_c14x2_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
        ...
        return encode_image_to_cmpr_block(
            pixels,
            colors_to_color_indexes,
            block_x,
            block_y,
...
def encode_image_to_rgb563_block(
    pixels,
    colors_to_color_indexes,
    block_x,
    block_y,
...
def encode_image_to_rgb5a3_block(
    pixels,
    colors_to_color_indexes,
    block_x,
    block_y,
...
def encode_image_to_rgba32_block(
    pixels,
    colors_to_color_indexes,
    block_x,
    block_y,
...
def encode_image_to_c14x2_block(
    pixels,
    colors_to_color_indexes,
    block_x,
    block_y,
...
def encode_image_to_cmpr_block(
    pixels,
    colors_to_color_indexes,
    block_x,
    block_y,
...
def color_exchange(
    ...
    if PY_FAST_TEXTURE_UTILS_INSTALLED:
        ...
        try:
            new_image_bytes = pyfasttextureutils.color_exchange(
                ...
                replacement_color,
                mask_bytes,
                validate_mask_colors,
                ignore_bright,
            )
        except Exception as e:
            if (
                str(e)
                == "Invalid color color in mask, only red (FF0000) and white (FFFFFF) should be present"
            ):
                ...
                invalid_colors_str = ", ".join(
                    "%02X%02X%02X%02X" % color for color in invalid_colors
                )
        ...
        new_image = Image.frombytes(
            image.mode, (image.width, image.height), new_image_bytes
        )
        return new_image
    ...
    replacement_h, replacement_s, replacement_v = colorsys.rgb_to_hsv(
        replacement_r / 255, replacement_g / 255, replacement_b / 255
    )
            ...
            if mask_path:
                if validate_mask_colors:
                    if mask_pixels[x, y] == (255, 0, 0, 255):
                        ...
                        raise Exception(
                            f"Invalid color {r:02X}{g:02X}{b:02X}{a:02X} in mask {mask_path}"
                        )
                else:
                    if mask_pixels[x, y] != (255, 0, 0, 255):
                        continue
